...
from twin4build.optimizer._single_shooting import CapturedControlObjective
...
class TestFunctionalControlObjective(unittest.TestCase):
    """Numerical-equivalence regression check for the optimizer's fast
    composed objective (selected on ``Simulator``).
    ...
    def setUpClass(cls):
        ...
        cls.simulator = tb.Simulator(model, execution_mode="functional")
    ...
    def _eval(self, theta_np, use_fast):
        ...
        opt._functional_objective = fast if use_fast else None
        try:
            self._bust_caches()
            f = float(opt._obj_ad(np.asarray(theta_np, dtype=np.float64)))
            g = np.asarray(opt._jac_ad(np.asarray(theta_np, dtype=np.float64)))
            return f, g
        finally:
            opt._functional_objective = fast
    ...
    def test_object_fallback_propagates_post_run_validation_failure(self):
        opt = self.optimizer
        simulator = opt.simulator
        functional = opt._functional_objective
        validator = simulator._validate_component_inputs

        def fail_after_run(model):
            raise ValueError("post-run non-finite input")

        opt._functional_objective = None
        simulator._validate_component_inputs = fail_after_run
        try:
            self._bust_caches()
            with self.assertRaisesRegex(ValueError, "post-run non-finite input"):
                opt._obj_ad(np.full(len(opt._theta_obj), 0.5))
        finally:
            simulator._validate_component_inputs = validator
            opt._functional_objective = functional

    def test_functional_objective_was_built(self):
        """The example model is composable, so the default fast path must not
        have silently fallen back."""
        self.assertIsNotNone(self.optimizer._functional_objective)

    @unittest.skipUnless(torch.cuda.is_available(), "CUDA unavailable")
    def test_slsqp_cuda_graph_bundle_replay_matches_eager(self):
        (
            model,
            waterflow,
            space_heater,
            _building_space,
            _costs,
            (_heating_setpoint, _cooling_setpoint, mf),
        ) = build_model()
        ...
        result = optimizer.optimize(
            ...
            step_size=2400,
            variables=[(waterflow, "scheduleValue", 0, mf)],
            objectives=[(waterflow, "scheduleValue", "min")],
            method=("scipy", "SLSQP", "ad"),
            options={"maxiter": 3},
        )
        stats = result.derivative_stats["scipy_value_gradient_bundle"]
        self.assertEqual(stats["captures"], 1)

        z = torch.as_tensor(result.x, dtype=torch.float64, device="cuda")
        eager_f, eager_g = optimizer._functional_objective.value_and_grad(z)
        self.assertAlmostEqual(float(result.fun), float(eager_f), places=8)
        np.testing.assert_allclose(result.jac, eager_g.cpu().numpy(), rtol=1e-8, atol=1e-10)

        evaluator = CapturedControlObjective(optimizer._functional_objective)
        z2 = torch.clamp(z + 0.1, 0.0, 1.0)
        try:
            evaluator.value_and_grad(z)
            graph_f, graph_g = evaluator.value_and_grad(z2)
            eager_f2, eager_g2 = optimizer._functional_objective.value_and_grad(z2)
            torch.testing.assert_close(graph_f, eager_f2)
            torch.testing.assert_close(graph_g, eager_g2)
            self.assertEqual(evaluator.stats["captures"], 1)
            self.assertEqual(evaluator.stats["replays"], 1)
        finally:
            evaluator.close()
