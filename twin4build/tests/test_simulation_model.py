...
class TestSimulationModel(unittest.TestCase):
    ...
    def test_add_connection(self):
        ...
        self.sim_model.add_connection(self.schedule, self.damper, "scheduleValue", "damperPosition")
        
        # Connection should be reflected in the graph structure
        # Check that damper is in schedule's outgoing connections
        self.assertEqual(self.damper, self.schedule.connected_through[0].connects_system_at[0].connection_point_of)
    ...
    def test_get_dir(self):
        """Test get_dir method."""
        path, isfile = self.sim_model.get_dir(folder_list=["test_folder"], filename="test.txt")
    ...
    def test_invalid_output_port(self):
        ...
        with self.assertRaises(AssertionError):
            self.sim_model.add_connection(
                self.schedule, self.damper,
                "invalid_output", "damperPosition"
            )
