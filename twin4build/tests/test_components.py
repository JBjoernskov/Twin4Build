...
class TestSpaceHeaterTorchSystem(unittest.TestCase):
    ...
    def test_do_step_batch(self):
        ...
        # Execute a time step
        datetime_val = datetime.datetime(2023, 1, 1, 0, 0, 0, tzinfo=pytz.UTC)
        heater_batch.do_step(second_time=0, date_time=datetime_val, step_size=step_size, step_index=0)
        ...
        self.assertIsNotNone(outlet_temp)
        self.assertIsNotNone(radiator_power)
        self.assertEqual(outlet_temp.shape[0], batch_size)  # Output batch matches input batch
        self.assertEqual(radiator_power.shape[0], batch_size)  # Output batch matches input batch
...
class TestCoilTorchSystem(unittest.TestCase):
    ...
    def test_do_step_batch(self):
        ...
        coil_batch.input["outletAirTemperatureSetpoint"].set(torch.tensor([22.0, 23.0]), step_index=0)
        ...
        self.assertIsNotNone(heating_power)
        self.assertIsNotNone(cooling_power)
        self.assertEqual(heating_power.shape[0], batch_size)  # Output batch matches input batch
        self.assertEqual(cooling_power.shape[0], batch_size)  # Output batch matches input batch
...
class TestAirToAirHeatRecoverySystem(unittest.TestCase):
    ...
    def test_do_step_batch(self):
        ...
        hr_batch.input["primaryTemperatureOutSetpoint"].set(torch.tensor([20.0, 20.0]), step_index=0)
        hr_batch.input["secondaryTemperatureIn"].set(torch.tensor([20.0, 22.0]), step_index=0)
        hr_batch.input["secondaryAirFlowRate"].set(torch.tensor([1.0, 0.8]), step_index=0)
        ...
        self.assertIsNotNone(primary_out)
        self.assertIsNotNone(secondary_out)
        self.assertEqual(primary_out.shape[0], batch_size)  # Output batch matches input batch
        self.assertEqual(secondary_out.shape[0], batch_size)  # Output batch matches input batch
...
class TestScheduleSystem(unittest.TestCase):
    ...
    def test_do_step_batch(self):
        ...
        end_time = [datetime.datetime(2023, 1, 1, 1, 40, 0, tzinfo=pytz.UTC)]
        step_size = [600]
        schedule_batch.initialize(start_time=start_time, end_time=end_time, step_size=step_size)
        
        # Execute a time step
        datetime_val = datetime.datetime(2023, 1, 1, 0, 30, 0, tzinfo=pytz.UTC)
        schedule_batch.do_step(second_time=1800, date_time=datetime_val, step_size=600, step_index=3)
...
class TestDamperTorchSystem(unittest.TestCase):
    ...
    def test_do_step_batch(self):
        ...
        # Execute a time step
        datetime_val = datetime.datetime(2023, 1, 1, 0, 0, 0, tzinfo=pytz.UTC)
        damper_batch.do_step(second_time=0, date_time=datetime_val, step_size=600, step_index=0)
        
        # Check that output was calculated - verify batch shape consistency
        airflow = damper_batch.output["airFlowRate"].get()
        self.assertIsNotNone(airflow)
        self.assertEqual(airflow.shape[0], batch_size)  # Output batch matches input batch
...
class TestFanTorchSystem(unittest.TestCase):
    ...
    def test_do_step_batch(self):
        ...
        fan_batch = FanTorchSystem(id="test_fan_batch", nominalPowerRate=1000.0, nominalAirFlowRate=1.0, c1=0, c2=0.8, c3=0.2, c4=0.0, f_total=0.9)
        ...
        end_time = [datetime.datetime(2023, 1, 1, 1, 40, 0, tzinfo=pytz.UTC)]
        step_size = [600]
        fan_batch.initialize(start_time=start_time, end_time=end_time, step_size=step_size)

        # Initialize inputs with batch size
        fan_batch.input["airFlowRate"].initialize(n_timesteps=1, batch_size=batch_size, size=1)
        fan_batch.input["inletAirTemperature"].initialize(n_timesteps=1, batch_size=batch_size, size=1)
        ...
        # Execute a time step
        datetime_val = datetime.datetime(2023, 1, 1, 0, 0, 0, tzinfo=pytz.UTC)
        fan_batch.do_step(second_time=0, date_time=datetime_val, step_size=600, step_index=0)
...
class TestValveTorchSystem(unittest.TestCase):
    ...
    def test_do_step_batch(self):
        ...
        valve_batch = ValveTorchSystem(id="test_valve_batch", valveAuthority=0.5, waterFlowRateMax=1.0)
        ...
        # Execute a time step
        datetime_val = datetime.datetime(2023, 1, 1, 0, 0, 0, tzinfo=pytz.UTC)
        valve_batch.do_step(second_time=0, date_time=datetime_val, step_size=600, step_index=0)
...
class TestOnOffControllerSystem(unittest.TestCase):
    ...
    def test_do_step_batch(self):
        ...
        controller_batch = OnOffControllerSystem(id="test_controller_batch", offValue=0, onValue=1, isReverse=False)
        ...
        controller_batch.input["setpointValue"].set(torch.tensor([22.0, 22.0]), step_index=0)
        
        # Execute a time step
        datetime_val = datetime.datetime(2023, 1, 1, 0, 0, 0, tzinfo=pytz.UTC)
        controller_batch.do_step(second_time=0, date_time=datetime_val, step_size=600, step_index=0)
...
class TestPIDControllerSystem(unittest.TestCase):
    ...
    def test_do_step_batch(self):
        ...
        controller_batch = PIDControllerSystem(id="test_pid_batch", Kp=1.0, Ki=0.1, Kd=0.01)
        ...
        controller_batch.input["setpointValue"].set(torch.tensor([22.0, 22.0, 22.0]), step_index=0)
...
class TestSensorSystem(unittest.TestCase):
    ...
    def test_do_step_batch(self):
        ...
        end_time = [datetime.datetime(2023, 1, 1, 1, 40, 0, tzinfo=pytz.UTC)]
        step_size = [600]
        sensor_batch.initialize(start_time=start_time, end_time=end_time, step_size=step_size)
        
        # Execute a time step
        datetime_val = datetime.datetime(2023, 1, 1, 0, 10, 0, tzinfo=pytz.UTC)
        sensor_batch.do_step(second_time=600, date_time=datetime_val, step_size=600, step_index=1)
...
class TestOutdoorEnvironmentSystem(unittest.TestCase):
    ...
    def test_initialize_with_df(self):
        ...
        outdoor_env.initialize(start_time=start_time, end_time=end_time, step_size=step_size)
    ...
    def test_do_step_without_correction(self):
        ...
        # Execute a time step
        datetime_val = datetime.datetime(2023, 1, 1, 0, 0, 0, tzinfo=pytz.UTC)
        outdoor_env.do_step(second_time=0, date_time=datetime_val, step_size=600, step_index=0)
...
class TestTimeSeriesInputSystem(unittest.TestCase):
    ...
    def test_do_step_batch(self):
        ...
        end_time = [datetime.datetime(2023, 1, 1, 1, 40, 0, tzinfo=pytz.UTC)]
        step_size = [600]
        ts_input_batch.initialize(start_time=start_time, end_time=end_time, step_size=step_size)
        
        # Execute a time step
        datetime_val = datetime.datetime(2023, 1, 1, 0, 10, 0, tzinfo=pytz.UTC)
        ts_input_batch.do_step(second_time=600, date_time=datetime_val, step_size=600, step_index=1)
...
class TestReturnFlowJunctionSystem(unittest.TestCase):
    ...
    def test_do_step(self):
        ...
        self.junction.input["airFlowRateIn"].initialize(n_timesteps=1, batch_size=1, size=2)
        self.junction.input["airTemperatureIn"].initialize(n_timesteps=1, batch_size=1, size=2)
        self.junction.output["airFlowRateOut"].initialize(n_timesteps=1, batch_size=1)
        self.junction.output["airTemperatureOut"].initialize(n_timesteps=1, batch_size=1)
    ...
    def test_do_step_batch(self):
        ...
        junction_batch = ReturnFlowJunctionSystem(id="test_return_junction_batch", airFlowRateBias=0.1)
        
        start_time = [datetime.datetime(2023, 1, 1, 0, 0, 0, tzinfo=pytz.UTC)]
        end_time = [datetime.datetime(2023, 1, 1, 1, 40, 0, tzinfo=pytz.UTC)]
        step_size = [600]
        batch_size = 2
        junction_batch.initialize(start_time=start_time, end_time=end_time, step_size=step_size)
        junction_batch.input["airFlowRateIn"].initialize(n_timesteps=1, batch_size=batch_size, size=2)
        junction_batch.input["airTemperatureIn"].initialize(n_timesteps=1, batch_size=batch_size, size=2)
        junction_batch.output["airFlowRateOut"].initialize(n_timesteps=1, batch_size=batch_size)
        junction_batch.output["airTemperatureOut"].initialize(n_timesteps=1, batch_size=batch_size)
        
        # Set inputs - vector inputs for multiple flows with batch size 2
        junction_batch.input["airFlowRateIn"].set(torch.tensor([[0.5, 0.5], [0.6, 0.4]]), step_index=0)
        junction_batch.input["airTemperatureIn"].set(torch.tensor([[20.0, 22.0], [21.0, 23.0]]), step_index=0)
        
        # Execute a time step
        datetime_val = datetime.datetime(2023, 1, 1, 0, 0, 0, tzinfo=pytz.UTC)
        junction_batch.do_step(second_time=0, date_time=datetime_val, step_size=600, step_index=0)
...
class TestSupplyFlowJunctionSystem(unittest.TestCase):
    ...
    def test_do_step(self):
        ...
        step_size = [600]
        self.junction.initialize(start_time=start_time, end_time=end_time, step_size=step_size)
        self.junction.input["airFlowRateOut"].initialize(n_timesteps=1, batch_size=1, size=3)
    ...
    def test_do_step_batch(self):
        ...
        junction_batch = SupplyFlowJunctionSystem(id="test_supply_junction_batch", airFlowRateBias=0.05)
        ...
        batch_size = 2
        junction_batch.initialize(start_time=start_time, end_time=end_time, step_size=step_size)
        junction_batch.input["airFlowRateOut"].initialize(n_timesteps=1, batch_size=batch_size, size=3)
        junction_batch.output["airFlowRateIn"].initialize(n_timesteps=1, batch_size=batch_size)

        # Set inputs - vector inputs for multiple flows with batch size 2
        junction_batch.input["airFlowRateOut"].set(torch.tensor([[0.3, 0.4, 0.3], [0.2, 0.5, 0.3]]), step_index=0)
        ...
        self.assertIsNotNone(flow_in)
        self.assertEqual(flow_in.shape[0], batch_size)  # Output batch matches input batch
...
class TestDiscreteStatespaceSystem(unittest.TestCase):
    ...
    def test_do_step(self):
        ...
        end_time = [datetime.datetime(2023, 1, 1, 1, 40, 0, tzinfo=pytz.UTC)]
        step_size = [600]
        self.system.initialize(start_time=start_time, end_time=end_time, step_size=step_size)
        
        # Set input
        self.system.input["u"].set(torch.tensor([[1.0]], dtype=torch.float64), step_index=0)
    ...
    def test_do_step_batch(self):
        ...
        # Execute a time step
        datetime_val = datetime.datetime(2023, 1, 1, 0, 0, 0, tzinfo=pytz.UTC)
        system_batch.do_step(second_time=0, date_time=datetime_val, step_size=step_size, step_index=0)
...
class TestPiecewiseLinearSystem(unittest.TestCase):
    def setUp(self):
        ...
        self.piecewise = PiecewiseLinearSystem(
            id="test_piecewise",
            X=X,
            Y=Y
        )
    ...
    def test_do_step(self):
        ...
        self.piecewise.initialize(start_time=start_time, end_time=end_time, step_size=step_size)
        
        # Set input (x value for interpolation)
        self.piecewise.input["x"].set(torch.tensor([2.5]), step_index=0)  # Midpoint between 0 and 5
    ...
    def test_do_step_batch(self):
        ...
        piecewise_batch.initialize(start_time=start_time, end_time=end_time, step_size=step_size)
...
class TestMaxSystem(unittest.TestCase):
    ...
    def test_do_step(self):
        ...
        self.max_system.initialize(start_time=start_time, end_time=end_time, step_size=step_size)
        ...
        # Execute a time step
        datetime_val = datetime.datetime(2023, 1, 1, 0, 0, 0, tzinfo=pytz.UTC)
        self.max_system.do_step(second_time=0, date_time=datetime_val, step_size=600, step_index=0)
...
class TestOnOffSystem(unittest.TestCase):
    ...
    def test_do_step_above_threshold(self):
        ...
        end_time = [datetime.datetime(2023, 1, 1, 1, 0, 0, tzinfo=pytz.UTC)]
        step_size = [600]
        self.onoff_system.initialize(start_time=start_time, end_time=end_time, step_size=step_size)
        
        # Set inputs - criteriaValue above threshold
        self.onoff_system.input["criteriaValue"].set(torch.tensor([1.0]), step_index=0)  # Above 0.5
        self.onoff_system.input["value"].set(torch.tensor([100.0]), step_index=0)
        
        # Execute a time step
        datetime_val = datetime.datetime(2023, 1, 1, 0, 0, 0, tzinfo=pytz.UTC)
        self.onoff_system.do_step(second_time=0, date_time=datetime_val, step_size=600, step_index=0)
    ...
    def test_do_step_below_threshold(self):
        ...
        self.onoff_system.input["criteriaValue"].set(torch.tensor([0.2]), step_index=0)  # Below 0.5
        self.onoff_system.input["value"].set(torch.tensor([100.0]), step_index=0)
    ...
    def test_do_step_batch(self):
        ...
        # Initialize inputs with batch size
        onoff_system_batch.input["criteriaValue"].initialize(n_timesteps=1, batch_size=batch_size, size=1)
        onoff_system_batch.input["value"].initialize(n_timesteps=1, batch_size=batch_size, size=1)
        
        # Set inputs with batch size 3 - some above, some below threshold
        onoff_system_batch.input["criteriaValue"].set(torch.tensor([1.0, 0.2, 0.8]), step_index=0)
        onoff_system_batch.input["value"].set(torch.tensor([100.0, 200.0, 150.0]), step_index=0)
        
        # Execute a time step
        datetime_val = datetime.datetime(2023, 1, 1, 0, 0, 0, tzinfo=pytz.UTC)
        onoff_system_batch.do_step(second_time=0, date_time=datetime_val, step_size=600, step_index=0)
...
class TestPassInputToOutput(unittest.TestCase):
    ...
    def test_do_step(self):
        ...
        end_time = [datetime.datetime(2023, 1, 1, 1, 0, 0, tzinfo=pytz.UTC)]
        step_size = [600]
        self.pass_system.initialize(start_time=start_time, end_time=end_time, step_size=step_size)
        ...
        # Execute a time step
        datetime_val = datetime.datetime(2023, 1, 1, 0, 0, 0, tzinfo=pytz.UTC)
        self.pass_system.do_step(second_time=0, date_time=datetime_val, step_size=600, step_index=0)
    ...
    def test_do_step_batch(self):
        ...
        end_time = [datetime.datetime(2023, 1, 1, 1, 0, 0, tzinfo=pytz.UTC)]
        step_size = [600]
        pass_system_batch.initialize(start_time=start_time, end_time=end_time, step_size=step_size)
        ...
        # Execute a time step
        datetime_val = datetime.datetime(2023, 1, 1, 0, 0, 0, tzinfo=pytz.UTC)
        pass_system_batch.do_step(second_time=0, date_time=datetime_val, step_size=600, step_index=0)
...
class TestBuildingSpaceTorchSystem(unittest.TestCase):
    ...
    def test_do_step(self):
        ...
        end_time = [datetime.datetime(2023, 1, 1, 1, 40, 0, tzinfo=pytz.UTC)]
        step_size = [600]
        self.building_space.initialize(start_time=start_time, end_time=end_time, step_size=step_size)
        
        # Set required inputs
        self.building_space.input["supplyAirFlowRate"].set(torch.tensor([0.5]), step_index=0)
        self.building_space.input["exhaustAirFlowRate"].set(torch.tensor([0.5]), step_index=0)
        self.building_space.input["outdoorTemperature"].set(torch.tensor([5.0]), step_index=0)
        self.building_space.input["supplyAirTemperature"].set(torch.tensor([20.0]), step_index=0)
        self.building_space.input["outdoorCO2"].set(torch.tensor([400.0]), step_index=0)
        self.building_space.input["numberOfPeople"].set(torch.tensor([2.0]), step_index=0)
        self.building_space.input["globalIrradiation"].set(torch.tensor([0.0]), step_index=0)
    ...
    def test_do_step_batch(self):
        ...
        self.assertIsNotNone(indoor_temp)
        self.assertIsNotNone(indoor_co2)
        self.assertEqual(indoor_temp.shape[0], batch_size)  # Output batch matches input batch
        self.assertEqual(indoor_co2.shape[0], batch_size)  # Output batch matches input batch
