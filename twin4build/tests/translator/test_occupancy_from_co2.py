...
room's CO2 sensor and the damper commands of the VAVs serving it (the
same point that drives the AHU branch flows),
...
class TestOccupancyFromCO2(unittest.TestCase):
    ...
    def test_one_occupancy_per_room_fed_by_its_sensors(self):
        ...
        for o in occ:
            ...
            self.assertTrue(all(f.uuid.endswith("_CMD") for f in positions), [f.uuid for f in positions])
    ...
    def test_room_with_two_vavs_gets_both_damper_commands(self):
        ...
        self.assertEqual(positions, ["R02_VAV01_CMD", "R02_VAV02_CMD"])
