...
import twin4build.simulator._functional as functional
...
class TestFunctionalEstimationObjective(unittest.TestCase):
    ...
    def setUpClass(cls):
        model = load_model()
        cls.estimator = tb.Estimator(tb.Simulator(model, execution_mode="functional"))
    ...
    def test_functional_objective_matches_object_graph(self):
        est = self.estimator
    ...
    def test_estimator_fast_option_is_removed(self):
        ...
        start = EXAMPLE_START[0]
        with self.assertRaisesRegex(TypeError, "Removed estimator option"):
            estimator.estimate(
...
class TestFunctionalEstimationObjectiveSharedTheta(TestFunctionalEstimationObjective):
    ...
    tests from :class:`TestFunctionalEstimationObjective`.
    """
...
def occupancy_theta_parameters(model):
    ...
    params = [
        p for p in example_parameters(model) if not (p[0] is space and p[1] == "mass.V")
    ]
    params.append(([space, occupancy], "mass.V", 80, 10, 300, "shared"))
    return params


class TestFunctionalEstimationObjectiveOccupancyTheta(
    TestFunctionalEstimationObjective
):
    ...
    Inherits both parity tests from :class:`TestFunctionalEstimationObjective`.
    """
...
class TestFunctionalEstimationObjectiveNonComposableTheta(unittest.TestCase):
    ...
    def test_falls_back_to_object_graph(self):
        ...
        saved = functional._has_real_forward
        functional._has_real_forward = lambda c: not isinstance(
            c, OccupancySystem
        ) and saved(c)
        try:
            estimator.estimate(
            ...
            functional._has_real_forward = saved
        self.assertIsNone(
