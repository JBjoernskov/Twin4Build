...
class TestEstimatorSaveLoad(unittest.TestCase):
    ...
    def test_estimator_save_and_load(self):
        ...
        parameters = [
            # Thermal parameters
            (space, "thermal.C_air", 2e+6, 1e+6, 1e+8),
            (space, "thermal.C_wall", 2e+6, 1e+6, 1e+8),
        ...
        self.assertIn("component_attr", result, "Result should contain 'component_attr'")
        ...
        model2.components["020B_temperature_sensor"].filename = utils.get_path(["estimator_example", "temperature_sensor.csv"])
        model2.components["020B_co2_sensor"].filename = utils.get_path(["estimator_example", "co2_sensor.csv"])
        model2.components["020B_valve_position_sensor"].filename = utils.get_path(["estimator_example", "valve_position_sensor.csv"])
        model2.components["020B_damper_position_sensor"].filename = utils.get_path(["estimator_example", "damper_position_sensor.csv"])
        model2.components["BTA004"].filename = utils.get_path(["estimator_example", "supply_air_temperature.csv"])
        model2.components["020B_temperature_heating_setpoint"].filename = utils.get_path(["estimator_example", "temperature_heating_setpoint.csv"])
        model2.components["outdoor_environment"].filename_outdoorTemperature = utils.get_path(["estimator_example", "outdoor_environment.csv"])
        model2.components["outdoor_environment"].filename_globalIrradiation = utils.get_path(["estimator_example", "outdoor_environment.csv"])
        model2.components["outdoor_environment"].valuecolumn_outdoorCo2Concentration = 2
        model2.components["outdoor_environment"].filename_outdoorCo2Concentration = utils.get_path(["estimator_example", "outdoor_environment.csv"])
        ...
        self.assertIsNotNone(model2.simulation_model._result, "Model should have a result after loading")
        self.assertIn("result_x", model2.simulation_model._result, "Loaded result should contain 'result_x'")
        ...
        test_start = datetime.datetime(
            year=2023, month=11, day=27, hour=0, minute=0, second=0,
            tzinfo=tz.gettz("Europe/Copenhagen")
        )
        test_end = datetime.datetime(
            year=2023, month=11, day=27, hour=1, minute=0, second=0,
            tzinfo=tz.gettz("Europe/Copenhagen")
        )
        ...
        self.assertAlmostEqual(space.thermal.C_air, space_loaded.thermal.C_air, delta=0.1*space.thermal.C_air)
        self.assertAlmostEqual(space.thermal.C_wall, space_loaded.thermal.C_wall, delta=0.1*space.thermal.C_wall)
        
        heater_loaded = model2.components["020B_space_heater"]
        self.assertAlmostEqual(heater_loaded.thermalMassHeatCapacity, space_heater.thermalMassHeatCapacity, delta=0.1*heater_loaded.thermalMassHeatCapacity)
