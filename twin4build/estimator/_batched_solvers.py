...
def solve_batched_multistart(
    ...
    *,
    ...
    ``chunk_solver(evaluator, start_chunk, lb, ub, options) -> dict`` replaces
    the built-in step for every chunk of starts: a plug-in step (see
    :mod:`twin4build.solvers.registry`) reuses this function's chunking,
    padding, start selection and result assembly.  The dict it returns
    carries one entry per start row for ``x``, ``fun``, ``success``,
    ``nonfinite_failed``, ``line_search_failed``, ``stalled``, ``nit``,
    ``nfev``, ``njev``, ``hessian_resets``, ``descent_fallbacks``,
    ``restoration_uses``, ``finite_candidates`` and ``last_alpha``, plus
    ``history`` and ``curvature_block_sizes``.  ``method`` then only names
    the step in the result.
    """
    ...
    for first in range(0, starts.shape[0], batch_size):
        ...
        if chunk_solver is not None:
            solved = chunk_solver(evaluator, start_chunk, lb_t, ub_t, dict(options))
        else:
            solved = _solve_chunk(
                evaluator,
                start_chunk,
