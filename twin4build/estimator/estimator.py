...
class Estimator():
        ...
        if add_gp:
            ...
            for j, measuring_device in enumerate(self.targetMeasuringDevices):
                ...
                if self.gp_add_time:
                    ...
                    add_lb[1:] = np.log(scale_lengths)-1
                    add_ub[1:] = np.log(scale_lengths)+1
    ...
    def _loglike_gaussian_process(self, theta):
        ...
        for measuring_device in self.targetMeasuringDevices:
            ...
            gp = george.GP(a*kernel, solver=george.HODLRSolver, tol=1e-5)#, white_noise=np.log(var))#(tol=0.01))
            gp.compute(x, std)
