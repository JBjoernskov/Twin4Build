...
def get_dict_differences(dict1: Dict[str, Any], dict2: Dict[str, Any], path: str = "") -> Dict[str, Any]:
    """
    ...
    if missing_in_2 or missing_in_1:
        ...
        if missing_in_2:
            result['missing_in_2'].update(f"{path}.{key}" if path else key for key in missing_in_2)
        if missing_in_1:
            result['missing_in_1'].update(f"{path}.{key}" if path else key for key in missing_in_1)
...
def merge_dicts(dict1: Dict[str, Any], dict2: Dict[str, Any], prioritize: str = None) -> Dict[str, Any]:
    """
