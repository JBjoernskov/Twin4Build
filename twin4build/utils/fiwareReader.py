...
class fiwareReader:
    ...
    def load_objects_from_fiware(self, accessToken: str, params: dict):
        ...
        headers = {'Authorization': 'Bearer ' + accessToken, 
                ...
                'Accept': '*/*'}
        resp = requests.get(url=self.config.fiwareBaseUrl + '/ngsi-ld/v1/entities', params=params, headers=headers)
        data = resp.json()
        return data
    ...
    def read_config_from_fiware(self):
            ...
            df_Systems = self.load_objects_of_type_from_fiware(access_token, "System")
            ...
            df_Space = self.load_objects_from_fiware(access_token, dict(type="BuildingSpace",options='keyValues',q='name=="Ø20-601b-2"'))
            df_Space_id_mapping = dict()
            df_Damper = self.load_objects_of_type_from_fiware(access_token, "Damper")
            df_SpaceHeater = self.load_objects_of_type_from_fiware(access_token, "SpaceHeater")
            ...
            df_AirToAirHeatRecovery = self.load_objects_of_type_from_fiware(access_token, "AirToAirHeatRecovery")
            df_Fan = self.load_objects_of_type_from_fiware(access_token, "Fan")
            df_Controller = self.load_objects_of_type_from_fiware(access_token, "Controller")
            
            for fiwareSystem in df_Systems:

                df_System_id_mapping[fiwareSystem["id"]] = fiwareSystem["name"]
                if fiwareSystem["purpose"] == "ventilation":
                    ventilation_system = DistributionDevice(subSystemOf = [], hasSubSystem = [], id = fiwareSystem["name"])
                    self.system_dict["ventilation"][fiwareSystem["name"]] = ventilation_system                    
                
                elif fiwareSystem["purpose"] == "heating":
                    heating_system = DistributionDevice(subSystemOf = [], hasSubSystem = [], id = fiwareSystem["name"])
                    self.system_dict["heating"][fiwareSystem["name"]] = heating_system

                elif fiwareSystem["purpose"] == "cooling":
                    cooling_system = DistributionDevice(subSystemOf = [], hasSubSystem = [], id = fiwareSystem["name"])
                    self.system_dict["cooling"][fiwareSystem["name"]] = cooling_system
            ...
            for row in df_Damper:
                ...
                if row["isContainedInBuildingSpace"] not in df_Space_id_mapping:
                    ...
                    damper = Damper(
                        ...
                        nominalAirFlowRate = Measurement(hasValue=row["nominalAirFlowRate"]),
                        connectedThrough = [],
                        connectsAt = [],
                        id = damper_name)
            ...
            for row in df_SpaceHeater:
                ...
                if row["isContainedInBuildingSpace"] not in df_Space_id_mapping:
                    ...
                    space_heater = SpaceHeater(
                        ...
                        thermalMassHeatCapacity = Measurement(hasValue=row["thermalMassHeatCapacity"]),
                        connectedThrough = [],
                        connectsAt = [],
            ...
            for row in df_Valve:
                ...
                if row["isContainedInBuildingSpace"] not in df_Space_id_mapping:
                    ...
                    valve = Valve(
                        subSystemOf = systems,
                        isContainedIn = self.component_base_dict[df_Space_id_mapping[row["isContainedInBuildingSpace"]]],
                        flowCoefficient = Measurement(hasValue=row["flowCoefficient"]),
            ...
            for row in df_AirToAirHeatRecovery:
                ...
                air_to_air_heat_recovery = AirToAirHeatRecovery(
                    subSystemOf = systems,
                    primaryAirFlowRateMax = Measurement(hasValue=row["primaryAirFlowRateMax"]),
                    secondaryAirFlowRateMax = Measurement(hasValue=row["secondaryAirFlowRateMax"]),
                    connectedThrough = [],
                    connectsAt = [],
