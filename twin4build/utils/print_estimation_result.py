...
def print_estimation_result(result: Union[str, Dict]) -> None:
    ...
    if isinstance(result, str):
        ...
        if ext != ".pickle":
            raise ValueError(
                f"The file {result} is not a pickle file. Expected .pickle extension."
            )
    ...
    assert isinstance(
        result, dict
    ), "Argument result must be a dictionary or dictionary-like object"
    ...
    assert (
        "component_attr" in result
    ), "Result dictionary must contain 'component_attr' key"
    ...
    assert len(result_x) == len(
        component_id
    ), "result_x and component_id must have the same length"
    assert len(result_x) == len(
        component_attr
    ), "result_x and component_attr must have the same length"
