...
from twin4build.translator.translator import Exact, Node, Optional_, SignaturePattern
...
def saref_signature_pattern():
    ...
    """
    ahu = Node(cls=core.namespace.S4BLDG.AirHandlingUnit)
    damper = Node(cls=core.namespace.S4BLDG.Damper)
    coil = Node(cls=core.namespace.S4BLDG.Coil)
    ...
    outdoor = Node(cls=core.namespace.S4BLDG.OutdoorEnvironment)
    space = Node(cls=core.namespace.S4BLDG.BuildingSpace)

    sp = SignaturePattern(id="air_handling_unit_signature_pattern")

    sp.add_triple(
        Exact(subject=damper, object=ahu, predicate=core.namespace.S4SYST.subSystemOf)
    )
    sp.add_triple(
        Exact(subject=coil, object=ahu, predicate=core.namespace.S4SYST.subSystemOf)
    )
    sp.add_triple(
        Exact(
            subject=heat_recovery,
            object=ahu,
            predicate=core.namespace.S4SYST.subSystemOf,
        )
    )
    sp.add_triple(
        Exact(subject=controller, object=damper, predicate=core.namespace.SAREF.controls)
    )
    sp.add_triple(
        Exact(subject=controller, object=coil, predicate=core.namespace.SAREF.controls)
    )
    sp.add_triple(
        Exact(subject=ahu, object=space, predicate=core.namespace.FSO.suppliesFluidTo)
    )
    sp.add_triple(
        Exact(
            subject=space,
    ...
    sp.add_triple(
        Exact(subject=ahu, object=outdoor, predicate=core.namespace.S4SYST.connectedTo)
    )
    sp.add_triple(
        Optional_(subject=ahu, object=schedule, predicate=core.namespace.SAREF.hasProfile)
    )

    sp.add_input("supplyDamperPosition", controller, "inputSignal")
    sp.add_input("exhaustDamperPosition", controller, "inputSignal")
    sp.add_input("exhaustTemperature", space, "indoorTemperature")
    sp.add_input("supplyAirTemperatureSetpoint", schedule, "scheduleValue")
    sp.add_input("outdoorAirTemperature", outdoor, "outdoorTemperature")

    sp.add_modeled_node(ahu)
    sp.add_modeled_node(damper)
    sp.add_modeled_node(coil)
    sp.add_modeled_node(heat_recovery)
    return sp


AirHandlingUnitTorchSystem.add_signature_pattern(saref_signature_pattern())
