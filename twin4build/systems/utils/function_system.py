...
class FunctionSystem(core.System):
    ...
    component supports functional execution: when
    ``Simulator(model, execution_mode="functional")`` is used, estimation,
    optimization, and the GPU-batched Pareto prepass thread ``fn`` directly
    into the functional one-step map (including under ``torch.func.vmap``), and
    gradients flow through it. Use differentiable primitives (``torch.relu``,
