...
class BuildingSpaceTorchSystem(core.System, nn.Module):
    ...
    def __init__(self,
                 ...
                 CO2_start: float = 400.0,    # Initial CO2 concentration [ppm]
                 
                 # Optional parameters
                 infiltration: float = 0.0,   # Air infiltration rate [m³/s]
                 airVolume: float = 100.0,    # Air volume [m³]
                 **kwargs):
        """
        ...
        Args:
            ...
            CO2_start: Initial CO2 concentration [ppm]
            T_boundary: Default boundary temperature [°C]
            infiltration: Air infiltration rate [m³/s]
            airVolume: Air volume [m³]
        ...
        self.input = {
            ...
            "airFlowRate": tps.Scalar(),       # Air flow rate [m³/s]
            ...
            "outdoorCo2Concentration": tps.Scalar(),  # Outdoor CO2 concentration [ppm]
    ...
    def _create_state_space_model(self):
        ...
        # Air flow rate
        B[0, 1] = 1.2 * 1005/self.C_air
        
        # Supply air temperature
        B[0, 2] = -1.2 * 1005/self.C_air  # T_air_supply coefficient
        
        # Solar radiation
        B[0, 3] = self.f_air/self.C_air  # Radiation coefficient for air
        B[1, 3] = self.f_wall/self.C_wall  # Radiation coefficient for wall
        
        # Number of people
        B[0, 4] = self.Q_occ_gain/self.C_air  # N_people coefficient
        
        # Outdoor CO2 concentration (not used in thermal model)
        # B[0, 5] = 0.0
        ...
        self.ss_model = DiscreteStatespaceSystem(
            A=A, B=B, C=C, D=D,
            x0=x0,
            state_names=None,  # Let the system generate default names
            add_noise=False,   # Disable noise
