...
def _require_casadi():
    ...
    if ca is None:  # pragma: no cover - exercised only w/o casadi
        raise ImportError(
...
def _map_options(options: Optional[Dict]) -> Dict:
    ...
    if options:
        raise TypeError(
            "Unsupported IPOPT option(s): " f"{', '.join(sorted(map(str, options)))}"
        )
    return ipopt_opts
...
def early_stopping_step(es: Dict, x: np.ndarray, f: float, viol: float) -> bool:
    """One early-stopping update for an IPOPT iterate; returns True to stop.

    ``es`` is the mutable state dict built in :func:`solve_ipopt_constrained`
    (``feas_tol``, ``patience``, ``min_delta_rel``, ``theta_tol``, ``n_theta``
    plus the running ``f_best`` / ``z_best`` / ``stall_f`` / ``stall_theta`` /
    ``stop_reason``).  Pure function of that state so the rules are testable
    without a solve.

    Rules:

    * Counting starts at the first FEASIBLE incumbent.  Before that (initial
      infeasibility reduction) the objective is evaluated off the trajectory
      manifold and is meaningless.
    * An INFEASIBLE iterate (restoration, a large step, a barrier decrease)
      neither resets nor advances the stagnation counters: its objective is
      scored with defect slack and is not comparable with the incumbent's.
      Counting these used to stop cold starts mid-descent -- on the canonical
      1-zone case IPOPT's iterates sit above ``feas_tol`` for ~50 iterations
      while the objective falls 9 -> 1.05, and the solve was cut at its first
      (poor) feasible incumbent.  ``max_iter`` still caps a solve that never
      becomes feasible again.
    * A feasible iterate resets the objective counter only if it beats the
      incumbent by ``min_delta_rel`` (relative), and the theta counter only if
      theta moved by more than ``theta_tol`` from the incumbent.
    """
    feasible = viol <= es["feas_tol"]
    if es["z_best"] is None:
        if feasible:
            es["f_best"] = f
            es["z_best"] = np.asarray(x, dtype=np.float64).copy()
        return False
    if not feasible:
        return False
    nt = es["n_theta"]
    improved_f = f < es["f_best"] - es["min_delta_rel"] * max(abs(es["f_best"]), 1e-12)
    moved_theta = float(np.abs(x[:nt] - es["z_best"][:nt]).max()) > es["theta_tol"]
    if f < es["f_best"]:
        es["f_best"] = f
        es["z_best"] = np.asarray(x, dtype=np.float64).copy()
    ...
    if es["stall_f"] >= es["patience"]:
        es["stop_reason"] = f"objective stagnant for {es['stall_f']} iterations"
        return True
    if es["stall_theta"] >= es["patience"]:
        es["stop_reason"] = f"theta stagnant for {es['stall_theta']} iterations"
        return True
    return False
...
def solve_ipopt_constrained(
    ...
    *,
    ...
    """Minimize ``fun`` subject to box bounds and ``lbg <= g(x) <= ubg``.

    ``lbg`` and ``ubg`` default to zero, preserving the equality-constraint
    API used by simultaneous transcription.  General bounds also support
    scalar inequalities without changing the callback/Jacobian bridge.
    ...
    lbg = (
        np.zeros(n_g, dtype=np.float64)
        if lbg is None
        else np.broadcast_to(np.asarray(lbg, dtype=np.float64), (n_g,)).copy()
    )
    ubg = (
        np.zeros(n_g, dtype=np.float64)
        if ubg is None
        else np.broadcast_to(np.asarray(ubg, dtype=np.float64), (n_g,)).copy()
    )
    if np.any(lbg > ubg):
        raise ValueError("Each constraint lower bound must be <= its upper bound")

    def _constraint_violation(values):
        values = np.asarray(values, dtype=np.float64).flatten()
        below = np.maximum(lbg - values, 0.0)
        above = np.maximum(values - ubg, 0.0)
        return float(np.maximum(below, above).max()) if values.size else 0.0
    ...
    if early_stopping is not None:
        ...
        if n_g:
            ...
            _viol0 = _constraint_violation(_g0)
        else:
            _viol0 = 0.0
        ...
        class _IterCB(ca.Callback):
            ...
            def eval(self, arg):
                x = np.asarray(arg[0]).flatten()
                f = float(arg[1])
                viol = _constraint_violation(np.asarray(arg[2])) if n_g else 0.0
                return [1.0 if early_stopping_step(es, x, f, viol) else 0.0]
    ...
    if es is not None:
        ...
        if es["z_best"] is not None:
            sol_viol = _constraint_violation(np.asarray(sol["g"])) if n_g else 0.0
