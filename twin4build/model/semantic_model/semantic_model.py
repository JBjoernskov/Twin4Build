...
class SemanticModel:
    ...
    def parse_namespaces(self, graph, namespaces=None):
        ...
        for prefix, uri in namespaces.items():
            try:
                ...
                warnings.warn(f"HTTPError: Cannot parse namespace {uri}. Sometimes this error occurs when the ontology is not available at the same address as the namespace.")
    ...
    def _handle_namespace_fallback(self, graph, prefix, uri):
        ...
        if graph==self.graph:
            if uri not in self.parsed_namespaces and uri not in self.error_namespaces:
                ...
                if hasattr(core.namespace, prefix.upper()) and hasattr(core.ontology, prefix.upper()):
                    if prefix.upper() in core.ontologies.namespaces:
                        ...
                        warnings.warn(f"Found namespace in core.ontology fallback: {fallback_ontology_uri}")
                        try:
                            ...
                            warnings.warn(f"Fallback namespace {fallback_ontology_uri} also failed: {str(e)}")
                else:
            ...
            if hasattr(core.namespace, prefix.upper()) and hasattr(core.ontology, prefix.upper()):
                if prefix.upper() in core.ontologies.namespaces:
                    fallback_namespace_uri = getattr(core.namespace, prefix.upper())
                    fallback_ontology_uri = getattr(core.ontology, prefix.upper())
                    warnings.warn(f"Found namespace in core.ontologies fallback: {fallback_namespace_uri}")
                    try:
                        graph.parse(fallback_ontology_uri)
                        return
                    except Exception as e:
    ...
    def visualize(self, query=None, include_full_uri=True):
        ...
        font_color_map = {"default": [white, white, None, None],
        ...
        font_size_map = {"default": [10, 8, 8, 6],
        ...
        font_bold_map = {"default": [True, True, None, None],
        ...
        for filename in os.listdir(dirname_ccomps):
            ...
            if os.path.isfile(file_path):
                ...
                dot_filename_dot = os.path.join(dirname_ccomps, filename.replace("ccomps", "dot"))
                args = [app_path,
                        "-q",
    ...
    def reason(self, namespaces=None):
        ...
        def build_equivalence_classes(ontology_graph: Graph) -> Dict[URIRef, Set[URIRef]]:
            """
            ...
            adjacency = {}
            for class1, _, class2 in ontology_graph.triples((None, URIRef("http://www.w3.org/2002/07/owl#equivalentClass"), None)):
        ...
        def propagate_class_assertions(equivalence_class: Set[URIRef], new_triples: set):
            """
    ...
    def serialize(self, folder_list: List[str] = None):
        dirname, _ = self.get_dir(folder_list=folder_list, filename="semantic_model.ttl")
