...
def fcn(self):
    ...
    self.components["supply_air_temperature_sensor"].filename = utils.get_path(["estimator_example", "supply_air_temperature.csv"])
...
model.semantic_model.visualize(format="png", instance_style=instance_style, deduplicate_inverse=True, include_full_uri=False, query=query, initial_node=initial_node, node_limit=35, traversal_mode="bfs", dpi=800)
model.simulation_model.visualize(format="png", instance_style=instance_style, deduplicate_inverse=True, include_full_uri=False, literals=False, dpi=800, compressed=True)
