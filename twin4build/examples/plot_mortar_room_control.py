    ...
    for s in sensors:
        if sensor_pattern.lower() in s.lower():
            ...
            if sensor_pattern == 'Zone_Air_Temperature_Sensor':
                ...
                return s
            elif sensor_pattern not in ['Zone_Air_Temperature_Sensor', 'Zone_Air_Temperature_Setpoint', 'Command']:
                return s
...
def plot_room_control(data: dict, room_id: str, ahu_id: str, save_path: Path = None):
    ...
    if 'zone_temp' in data:
        ...
        if idx is not None:
            ax1.plot(idx, vals, color=colors['zone_temp'], 
                     label='Zone Temp', linewidth=1, alpha=0.8)
    ...
    if 'ahu_supply_air_temp' in data:
        ...
        if idx is not None:
            ax1.plot(idx, vals, color=colors['supply_temp'], 
                     label='AHU Supply Temp', linewidth=1, alpha=0.6)
    
    if 'ahu_outside_air_temp' in data or 'outside_temp' in data:
        key = 'ahu_outside_air_temp' if 'ahu_outside_air_temp' in data else 'outside_temp'
        idx, vals = get_values_safe(data[key])
        if idx is not None:
            ax1.plot(idx, vals, color=colors['outside_temp'], 
                     label='Outside Temp', linewidth=1, alpha=0.5)
    ...
    if 'heating_mode' in data:
        ...
        if idx is not None:
            ax3_twin = ax3.twinx()
            ax3_twin.plot(idx, vals, color=colors['heating'], 
                          label='Heating Mode', linewidth=1, alpha=0.7)
    ...
    if 'supply_air_flow' in data:
        ...
        if idx is not None:
            ax4.plot(idx, vals, color=colors['flow'], 
                     label='Supply Air Flow', linewidth=1)
...
Examples:
  ...
  python plot_mortar_room_control.py --save room_control.png
    ...
    if result.returncode != 0 or not result.stdout.strip():
        ...
        print(f"  python unified_ingest_streaming.py --dataset mortar --building bldg34")
        return
