...
def load_stream(sensor_name: str, start_time: datetime, end_time: datetime, 
                table_name: str = TABLE_NAME, step_size: int = 900):
...
def fetch_room_data(start_time: datetime, end_time: datetime,
...
def plot_room_control(data: dict, room_id: str, ahu_id: str, save_path: Path = None):
    ...
    fig.suptitle(f'Room {room_id} Temperature Control (AHU: {ahu_id})\nBuilding: bldg1 (Mortar Dataset)', 
    ...
    if 'zone_temp' in data:
        ...
        if idx is not None:
            ax1.plot(idx, vals, color=colors['zone_temp'], 
                     label='Zone Temp', linewidth=1.2, alpha=0.9)
    
    if 'zone_control_temp' in data:
        idx, vals = get_values_safe(data['zone_control_temp'])
        if idx is not None:
            ax1.plot(idx, vals, color=colors['control_temp'], 
                     label='Control Temp', linewidth=1, alpha=0.6, linestyle=':')
    
    if 'zone_temp_setpoint' in data:
        idx, vals = get_values_safe(data['zone_temp_setpoint'])
        if idx is not None:
            ax1.plot(idx, vals, color=colors['setpoint'], 
                     label='Setpoint', linewidth=1.5, linestyle='--')
    ...
    if 'supply_air_temp' in data:
        ...
        if idx is not None:
            ax1.plot(idx, vals, color=colors['vav_supply_temp'], 
                     label='VAV Supply Temp', linewidth=1.2, alpha=0.8)
    
    if 'ahu_supply_air_temp' in data:
        idx, vals = get_values_safe(data['ahu_supply_air_temp'])
        if idx is not None:
            ax1.plot(idx, vals, color=colors['ahu_supply_temp'], 
                     label='AHU Supply Temp', linewidth=1, alpha=0.5)
    
    if 'ahu_outside_air_temp' in data:
        idx, vals = get_values_safe(data['ahu_outside_air_temp'])
        if idx is not None:
            ax1.plot(idx, vals, color=colors['outside_temp'], 
                     label='Outside Temp', linewidth=1, alpha=0.4)
    ...
    if 'damper_position' in data:
        ...
        if idx is not None:
            ax2.plot(idx, vals, color=colors['damper'], 
                     label='Damper Position', linewidth=1)
    ...
    if 'reheat_valve' in data:
        ...
        if idx is not None:
            ax3.plot(idx, vals, color=colors['valve'], 
                     label='Reheat Valve', linewidth=1)
    ...
    if 'supply_air_flow' in data:
        ...
        if idx is not None:
            ax4.plot(idx, vals, color=colors['flow'], 
                     label='Supply Air Flow (CFM)', linewidth=1)
    ...
    if 'percent_air_flow' in data:
        ...
        if idx is not None:
            ax4_twin.plot(idx, vals, color=colors['percent_flow'], 
                          label='% of Design', linewidth=1, alpha=0.7, linestyle='--')
    ...
    if 'ahu_cooling_valve' in data:
        ...
        if idx is not None:
            ax5.plot(idx, vals, color='#3498db', 
                     label='Cooling Valve', linewidth=1)
...
Examples:
  ...
  python plot_mortar_bldg1_room.py --save room_control.png
    ...
    parser.add_argument('--list', action='store_true',
                        help='List available rooms and sensors')
