...
def generate_data_with_twin4build(
    ...
    print(
        f"   Heater power range: {heater_power.min():.0f}W to {heater_power.max():.0f}W"
    )
...
def run_controller_identification_example():
    ...
    ax1.plot(
        time_hours,
        df_temperature["value"].values,
        "b-",
        linewidth=1,
        label="Indoor Temperature",
    )
    ax1.plot(
        time_hours, df_setpoint["value"].values, "r--", linewidth=2, label="Setpoint"
    )
    ax1.plot(
        time_hours,
        weather["outdoor_temp"]["value"].values,
        "g-",
        alpha=0.7,
        linewidth=1,
    ...
    ax1.set_title(
        "Generated Synthetic Data - Closed Loop Simulation\n"
    ...
    ax4_twin.plot(
        time_hours,
        weather["occupancy"]["value"].values,
        "c-",
        linewidth=1,
        label="Occupancy",
    )
    ...
    temperature_schedule = tb.ScheduleSystem(filename=temp_file, id="temperature_input")
    ...
    for p in parameters:
        print(
            f"     - {p[0].id if hasattr(p[0], 'id') else p[0].__class__.__name__}.{p[1]} x0:{p[2]} lb:{p[3]} ub:{p[4]}"
        )
    ...
    ax2.plot(
        time_hours, final_predictions, "m-", linewidth=1, label="Identified prediction"
    )
