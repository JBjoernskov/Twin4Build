...
def generate_data_with_twin4build(
    ...
    valve = tb.ValveTorchSystem(waterFlowRateMax=0.05, valveAuthority=0.5, id="heating_valve")
    
    space_heater = tb.SpaceHeaterTorchSystem(
        Q_flow_nominal_sh=2000.0, T_a_nominal_sh=55.0,
        T_b_nominal_sh=45.0, TAir_nominal_sh=20.0, id="radiator"
    )
    ...
    # Add components
    for comp in [building_space, valve, space_heater, pi_controller,
    ...
    return df_temperature, df_setpoint, df_actuator, weather, start_time, end_time, {
...
def run_multi_signal_identification_example():
    ...
    df_temperature, df_setpoint, df_actuator, weather, start_time, end_time, true_params = \
    ...
    sensors, setpoints = create_decoy_signals(df_temperature, df_setpoint, weather, n_timesteps)
    
    print("\n   SENSORS:")
    for i, (df, name) in enumerate(sensors):
        print(f"     [{i}] {name}: range {df['value'].min():.1f} to {df['value'].max():.1f}")
    ...
    for i, (df, name) in enumerate(sensors):
        ...
        ax1.plot(time_hours, df['value'].values, style, color=colors[i], 
                 alpha=alpha, linewidth=1, label=name)
    ...
    # Connect output
    model.add_connection(controller, actuator_sensor, "inputSignal", "measuredValue", output_port_index=0)
    ...
    print(f"\n   Initial weights:")
    beta_vals_init = [f"{controller._get_beta(i).item():.3f}" for i in range(len(sensors))]
    gamma_vals_init = [f"{controller._get_gamma(i).item():.3f}" for i in range(len(setpoints))]
    ...
    for i, (df, name) in enumerate(sensors):
        ...
        status = "✓" if (("TRUE" in name and beta > 0.5) or ("DECOY" in name and beta < 0.5)) else "✗"
    ...
    for i, (df, name) in enumerate(setpoints):
        ...
        expected = "≈1.0" if "TRUE" in name else "≈0.0"
        status = "✓" if (("TRUE" in name and gamma > 0.5) or ("DECOY" in name and gamma < 0.5)) else "✗"
        print(f"     gamma_{i} = {gamma:.4f} (expected {expected}) [{name}] {status}")
    
    print("\n   Controller parameters:")
    print(f"     kp = {controller.candidate_0_0.kp.get().item():.6f} (true: {true_params['kp']})")
    print(f"     Ti = {controller.candidate_0_0.Ti.get().item():.6f} (true: {true_params['Ti']})")
    ...
    ax1.plot(time_hours, initial_predictions, 'b--', alpha=0.5, linewidth=1, label='Initial')
    ...
    bars2 = ax2.bar(x_gamma, gamma_vals, color='red', alpha=0.7, label='Gamma (setpoints)')
    ...
    ax2.set_ylim(0, 1.1)
    ax2.legend(loc='upper right')
    ax2.set_title('Signal Selection Weights (TRUE signals should be ≈1.0, DECOY signals should be ≈0.0)')
