...
actuator_0 = tb.SensorSystem(
    uuid=FCU1["valve_pos_1"],
    id="fcu1_valve1_actuator",
    dbconfig=db_config,
    transformation=transformation,
)
actuator_1 = tb.SensorSystem(
    uuid=FCU1["valve_pos_2"],
    id="fcu1_valve2_actuator",
    dbconfig=db_config,
    transformation=transformation,
)
actuator_2 = tb.SensorSystem(
    uuid=FCU2["valve_pos_1"],
    id="fcu2_valve1_actuator",
    dbconfig=db_config,
    transformation=transformation,
)
actuator_3 = tb.SensorSystem(
    uuid=FCU2["valve_pos_2"],
    id="fcu2_valve2_actuator",
    dbconfig=db_config,
    transformation=transformation,
)
...
for i, actuator in enumerate(actuator_sensors):
    model.add_connection(
        controller, actuator, "inputSignal", "measuredValue", output_port_index=i
    )
...
for i in range(len(actuator_sensors)):
    ...
    print(
        f"    Actuator {i}: {improvement:.1f}% improvement (MAE: {mae_initial:.4f} → {mae_final:.4f})"
    )
