...
def solve_equilibrium_differentiable_old(
    model, fan_speed, supply_damper_pos, exhaust_damper_pos, n_iterations=30000
):
    """
    ...
    for i in range(n_iterations):
        ...
        flow = (
            ...
            + step_size
            * pressure_error
            / (model.fan.design_pressure + 1e-12)
            * model.fan.design_flow
        )
...
class DamperModel(nn.Module):
    ...
    def __init__(self, fully_open_resistance: float, fully_closed_resistance: float):
        super().__init__()
        self.log_R_open = nn.Parameter(
            torch.tensor(np.log(fully_open_resistance), dtype=torch.float32)
        )
        self.k = nn.Parameter(
            torch.tensor(
...
class BranchModel(nn.Module):
    ...
    def __init__(
        ...
        self.log_R_duct = nn.Parameter(
            torch.tensor(np.log(resistance_coeff), dtype=torch.float32)
        )
        self.R_linear = nn.Parameter(
            torch.tensor(linear_resistance, dtype=torch.float32)
        )
    ...
    def get_pressure_drop(
        ...
        dp_duct = (
            R_duct * abs_flow**self.flow_exponent + torch.abs(self.R_linear) * abs_flow
        )
...
class AirNetworkPyTorch(nn.Module):
    ...
    def __init__(
        self,
        fan: FanModel,
        supply_branches: List[BranchModel],
...
def train_model(
    ...
    exhaust_dampers = torch.tensor(
        measured_data["exhaust_dampers"], dtype=torch.float32
    )
    measured_flow = torch.tensor(measured_data["measured_flow"], dtype=torch.float32)
    measured_power = torch.tensor(
        measured_data.get("measured_power", [0] * len(measured_flow)),
        dtype=torch.float32,
    )
...
if __name__ == "__main__":
    ...
    print(
        f"Fan curve: a={model.fan.a.item():.2f}, b={model.fan.b.item():.2f}, c={model.fan.c.item():.4f}"
    )
    ...
    print(
        f"Fan curve: a={true_fan.a.item():.2f}, b={true_fan.b.item():.2f}, c={true_fan.c.item():.4f}"
    )
    ...
    axes[0, 1].plot(
        [measured_flows.min(), measured_flows.max()],
        [measured_flows.min(), measured_flows.max()],
        "r--",
        label="Perfect",
    )
    ...
    axes[0, 2].plot(
        [measured_powers.min(), measured_powers.max()],
        [measured_powers.min(), measured_powers.max()],
        "r--",
        label="Perfect",
    )
    ...
    with torch.no_grad():
        learned_pressures = model.fan.get_pressure_rise(
            flows_tensor, speed_tensor
        ).numpy()
