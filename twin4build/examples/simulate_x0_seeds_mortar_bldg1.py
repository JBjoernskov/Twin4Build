...
# ---------------------------------------------------------------------------
LOGGER.logfile = "simulate_x0_seeds_mortar_bldg1.log"
...
# Reuse the existing serialized stage-1 model when present, otherwise translate
LOAD_SERIALIZED_MODEL = True
...
def _prepare_stage1_model(sim_model):
    ...
    for c in sim_model.components.values():
        if isinstance(c, ControllerIdentificationPISystem) and not c._built:
            ...
            c.n_actuators = c._get_n_actuators_from_connections() or 1
            c._build_components()
    ...
    for comp in sim_model.components.values():
...
def _build_stage1_model():
    sim_model = tb.SimulationModel(id=STAGE1_MODEL_ID)
    stage1_rdf, exists = sim_model.get_dir(
        folder_list=["semantic_model"], filename="instance_graph.ttl"
    )
...
def main():
    ...
    # rewire_pi_loops mutates the graph (remove_connection / CITS rebuild) and
    # clears _is_loaded.  Re-load before simulating.
    if any(r.pruned for r in rewire_reports.values()):
        sim_model.load()
    ...
    # (mirror translator example, including the GMM-derived
    # gamma_gate / gate_0.threshold / gate_0.band for high-conf CITS).
    _apply_frozen_weights(cits_components, sim_model, rewire_reports)
    ...
    fig_path = out_dir / "simulate_x0_seeds_mortar_bldg1.png"
    inputs_fig_path = out_dir / "simulate_x0_seeds_mortar_bldg1_inputs.png"
