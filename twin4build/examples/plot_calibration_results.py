...
def add_hourly_ticks(axes, tz_info, hour_interval=6):
    ...
    for ax in axes:
        ...
        ax.xaxis.set_minor_locator(mdates.HourLocator(interval=hour_interval, tz=tz_info))
...
def compute_metrics(measured, simulated, name=""):
    ...
    print(f"  {name:<30s}  RMSE={rmse:8.4f}  MAE={mae:8.4f}  ME={me:+8.4f}  R²={r2:.4f}  CV-RMSE={cvrmse:.1f}%")
    return {"rmse": rmse, "mae": mae, "me": me, "r2": r2, "cvrmse": cvrmse}


def move_legend(fig, y=1.0, fontsize=11, labelspacing=0.2, handlelength=1.5,
                columnspacing=1.0, ncol=3):
...
def main():
    ...
    temp_sim  = model.components["office_temperature_sensor"].output["measuredValue"].history()
    ...
    valve_meas = model.components["office_valve_position_sensor"].time_series_input.values
    valve_sim  = model.components["office_valve_position_sensor"].output["measuredValue"].history()
    damper_meas = model.components["office_damper_position_sensor"].time_series_input.values
    damper_sim  = model.components["office_damper_position_sensor"].output["measuredValue"].history()
    heating_setpoint = model.components["office_temperature_heating_controller"].input["setpointValue"].history()
    ...
    fig, axes = tb.plot.plot(
        dt,
        [
            tb.plot.Entry(temp_sim,        label="Temperature simulated", color="#d95f02", linewidth=2),
            tb.plot.Entry(temp_meas,       label="Temperature measured",  color=C.green, linewidth=1.5, fmt="--"),
            tb.plot.Entry(valve_sim,       label="Valve simulated",       color="#e7298a", linewidth=2, axis=2),
            tb.plot.Entry(valve_meas,      label="Valve measured",        color="#1f78b4", linewidth=1.5, fmt="--", axis=2),
            tb.plot.Entry(heating_setpoint, label="Heating setpoint",     color=C.black, linewidth=1.5, fmt=":"),
        ],
    ...
    fig, axes = tb.plot.plot(
        dt,
        [
            tb.plot.Entry(co2_sim,     label=r"CO$_2$ simulated", color="#d95f02", linewidth=2),
            tb.plot.Entry(co2_meas,    label=r"CO$_2$ measured",  color=C.green, linewidth=1.5, fmt="--"),
            tb.plot.Entry(damper_sim,  label="Damper simulated",  color="#e7298a", linewidth=2, axis=2),
            tb.plot.Entry(damper_meas, label="Damper measured",   color="#1f78b4", linewidth=1.5, fmt="--", axis=2),
            tb.plot.Entry(occ_signal, label="Occupancy signal (binary)",     color=C.black, linewidth=1.5, fmt=":", axis=2),
            tb.plot.Entry(n_people, label="Occupancy count",     color=C.blue, linewidth=1.5, fmt=":", axis=3),
        ],
        ylim_1axis=(400, 800),
        ylim_2axis=(0, 1.5),
        ylim_3axis=(0, 6),
    ...
    fig, axes = tb.plot.plot(
        dt,
        [
            tb.plot.Entry(n_people,   label="Estimated occupancy",       color="#d95f02", linewidth=1.5),
            tb.plot.Entry(occ_signal, label="Occupancy signal (binary)", color="#7570b3", linewidth=1.5, axis=2),
            tb.plot.Entry(damper_sim, label="Damper simulated",          color="#e7298a", linewidth=1.5, axis=2),
        ],
        ylim_2axis=(0, 1),
    ...
    energy_kwh = model.components["office_space_heater"].output["Power"].history().sum() * step_size / 3600 / 1000
