...
def generate_single_controller_data(
    ...
    model.add_connection(
        setpoint_schedule, controller, "scheduleValue", "setpointValue"
    )
    ...
    model.add_connection(
        supply_water_temp_schedule, heater, "scheduleValue", "supplyWaterTemperature"
    )
    model.add_connection(
...
def generate_dual_actuator_data(
    ...
    print(
        f"   On-Off Temperature range: {temp_onoff.min():.1f}°C to {temp_onoff.max():.1f}°C"
    )
    print(f"   PI Actuator range: {actuator_pi.min():.2f} to {actuator_pi.max():.2f}")
    print(
        f"   On-Off Actuator range: {actuator_onoff.min():.2f} to {actuator_onoff.max():.2f}"
    )
...
def create_complex_decoy_signals(df_temp_pi, df_temp_onoff, df_setpoint, weather):
    ...
    timestamps = df_temp_pi[
        "datetime"
    ].tolist()  # Preserves timezone-aware datetime objects
    ...
    sensors.append((weather["outdoor_temp"].copy(), "outdoor_temp_DECOY"))
    ...
    setpoints.append(
        (pd.DataFrame({"datetime": timestamps, "value": inverted_sp}), "inverted_DECOY")
    )
    ...
    setpoints.append(
        (pd.DataFrame({"datetime": timestamps, "value": delayed_sp}), "delayed_DECOY")
    )
...
def run_complex_identification_example():
    ...
    for i, (df, name) in enumerate(sensors):
        ...
        print(
            f"     {marker} [{i}] {name}: range {df['value'].min():.1f} to {df['value'].max():.1f}"
        )
    ...
    for i, (desc, kwargs) in enumerate(zip(candidate_descriptions, controller_kwargs)):
        ...
        if ctrl_type == "PID":
            print(
                f"     [{i}] {desc}: Kp={kwargs['kp']}, Ti={kwargs['Ti']}, Td={kwargs['Td']}"
            )
        else:
            print(
                f"     [{i}] {desc}: off={kwargs['offValue']}, on={kwargs['onValue']}, k={kwargs['steepness']}"
            )
    ...
    ax4.plot(
        time_hours,
        df_actuator_onoff["value"].values,
        "r-",
        linewidth=1.5,
        label="On-Off Controller",
    )
    ...
    ax5.plot(
        time_hours,
        sensors[0][0]["value"].values,
        "b-",
        linewidth=1.5,
        label="Indoor Temp (PI sim)",
        alpha=0.8,
    )
    ax5.plot(
        time_hours,
        sensors[1][0]["value"].values,
        "g-",
        linewidth=1.5,
        label="Indoor Temp (On-Off sim)",
        alpha=0.8,
    )
    ax5.plot(
        time_hours,
        setpoints[0][0]["value"].values,
        "r--",
        linewidth=2,
        label="Setpoint",
    )
    ...
    actuator_sensor_onoff = tb.SensorSystem(
        filename=act_onoff_file, id="actuator_sensor_onoff"
    )
    ...
    initial_pred_pi = (
        actuator_sensor_pi.input["measuredValue"].history(i_s=0, i_c=0).detach().numpy()
    )
    initial_pred_onoff = (
        actuator_sensor_onoff.input["measuredValue"]
        .history(i_s=0, i_c=0)
        .detach()
        .numpy()
    )
    ...
    print(f"\n   Initial weights:")
    print(
        f"     Alpha Act.0:        {[f'{controller._get_alpha(0, c).item():.3f}' for c in range(len(controller_classes))]}"
    )
    print(
        f"     Beta Act.0:         {[f'{controller._get_beta(0, i).item():.3f}' for i in range(len(sensors))]}"
    )
    print(
        f"     Gamma Act.0:        {[f'{controller._get_gamma(0, i).item():.3f}' for i in range(len(setpoints))]}"
    )
    print(
        f"     Alpha Act.1:        {[f'{controller._get_alpha(1, c).item():.3f}' for c in range(len(controller_classes))]}"
    )
    print(
        f"     Beta Act.1:         {[f'{controller._get_beta(1, i).item():.3f}' for i in range(len(sensors))]}"
    )
    print(
        f"     Gamma Act.1:        {[f'{controller._get_gamma(1, i).item():.3f}' for i in range(len(setpoints))]}"
    )
    ...
    for comp, attr, x0, lb, ub, *_ in parameters:
        if isinstance(x0, (list, np.ndarray)):
            ...
            n_vals = 1
        print(
            f"     theta[{theta_idx}:{theta_idx+n_vals}] -> {attr} (x0={x0}, lb={lb}, ub={ub})"
        )
        theta_idx += n_vals
    ...
    def _debug_obj(theta_tensor, output="scalar"):
        ...
        if _debug_iter[0] == 0:
            ...
            pred_pi = (
                actuator_sensor_pi.input["measuredValue"]
                .history(i_s=0, i_c=0)
                .detach()
                .numpy()
            )
            ...
            sensor_vals = (
                controller.input["sensorValue"].history(i_s=0, i_c=0).detach().numpy()
            )
            setpoint_vals = (
                controller.input["setpointValue"].history(i_s=0, i_c=0).detach().numpy()
            )
            print(f"    Raw sensor input shape:   {sensor_vals.shape}")
            print(f"    Raw setpoint input shape: {setpoint_vals.shape}")
            print(
                f"    Sensor[0] (first 10):     {sensor_vals[:10, 0] if sensor_vals.ndim > 1 else sensor_vals[:10]}"
            )
            print(
                f"    Setpoint[0] (first 10):   {setpoint_vals[:10, 0] if setpoint_vals.ndim > 1 else setpoint_vals[:10]}"
            )
    ...
    for a in range(2):
        ...
        for i, (df, name) in enumerate(sensors):
            ...
            correct = (i == expected_sensor and beta > 0.5) or (
                i != expected_sensor and beta < 0.5
            )
            status = "✓ CORRECT" if correct else "✗ WRONG"
            print(
                f"     β_{a},{i} = {beta:.4f} (expected {expected}) [{name}] {status}"
            )
    ...
    for a in range(2):
        ...
        for i, (df, name) in enumerate(setpoints):
            ...
            correct = ("TRUE" in name and gamma > 0.5) or (
                "DECOY" in name and gamma < 0.5
            )
            status = "✓ CORRECT" if correct else "✗ WRONG"
            print(
                f"     γ_{a},{i} = {gamma:.4f} (expected {expected}) [{name}] {status}"
            )
    ...
    ax1.plot(
        time_hours, initial_pred_pi, "b--", alpha=0.5, linewidth=1, label="Initial"
    )
    ...
    ax2.plot(
        time_hours, initial_pred_onoff, "b--", alpha=0.5, linewidth=1, label="Initial"
    )
    ...
    alpha_vals_0 = [
        controller._get_alpha(0, c).item() for c in range(len(controller_classes))
    ]
    ...
    alpha_vals_1 = [
        controller._get_alpha(1, c).item() for c in range(len(controller_classes))
    ]
    ...
    ax7.plot(
        ...
        "b-",
        alpha=0.7,
        label=f"Actuator 0 (PI) MAE: {error_pi.mean():.4f}",
    )
    ax7.plot(
        time_hours,
        error_onoff,
        "r-",
        alpha=0.7,
        label=f"Actuator 1 (On-Off) MAE: {error_onoff.mean():.4f}",
    )
    ...
    n_correct_beta_0 = sum(
        1
        for i in range(len(sensors))
        if ((i == 0 and beta_vals_0[i] > 0.5) or (i != 0 and beta_vals_0[i] < 0.5))
    )
    n_correct_beta_1 = sum(
        1
        for i in range(len(sensors))
        if ((i == 1 and beta_vals_1[i] > 0.5) or (i != 1 and beta_vals_1[i] < 0.5))
    )
    ...
    n_correct_gamma_0 = sum(
        1
        for i, (df, name) in enumerate(setpoints)
        if (
            ("TRUE" in name and gamma_vals_0[i] > 0.5)
            or ("DECOY" in name and gamma_vals_0[i] < 0.5)
        )
    )
    n_correct_gamma_1 = sum(
        1
        for i, (df, name) in enumerate(setpoints)
        if (
            ("TRUE" in name and gamma_vals_1[i] > 0.5)
            or ("DECOY" in name and gamma_vals_1[i] < 0.5)
        )
    )
    n_correct_setpoints = n_correct_gamma_0 + n_correct_gamma_1

    # Actuator 0: Should select candidate 0 (PI)
    n_correct_ctrl_0 = sum(
        1
        for c in range(len(controller_classes))
        if ((c == 0 and alpha_vals_0[c] > 0.5) or (c != 0 and alpha_vals_0[c] < 0.5))
    )

    # Actuator 1: Should select candidate 4 (On-Off)
    n_correct_ctrl_1 = sum(
        1
        for c in range(len(controller_classes))
        if ((c == 1 and alpha_vals_1[c] > 0.5) or (c != 1 and alpha_vals_1[c] < 0.5))
    )
    ...
    print(
        f"   Gamma selection (setpoints): {n_correct_setpoints}/{2 * len(setpoints)} correct (2 actuators)"
    )
    print(
        f"   Actuator 0 controller:  {n_correct_ctrl_0}/{len(controller_classes)} correct"
    )
    print(
        f"   Actuator 1 controller:  {n_correct_ctrl_1}/{len(controller_classes)} correct"
    )

    total_correct = (
        n_correct_sensors + n_correct_setpoints + n_correct_ctrl_0 + n_correct_ctrl_1
    )
