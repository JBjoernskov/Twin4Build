...
start_time = [datetime(2022, 2, 1, 20, tzinfo=timezone.utc),
...
print(f"  Room temp range: [{np.min(room_temp_vals):.2f}, {np.max(room_temp_vals):.2f}]°C")
...
actuator_0 = tb.SensorSystem(uuid=FCU1['valve_pos_1'], id="fcu1_valve1_actuator", dbconfig=db_config)
actuator_1 = tb.SensorSystem(uuid=FCU1['valve_pos_2'], id="fcu1_valve2_actuator", dbconfig=db_config)
actuator_2 = tb.SensorSystem(uuid=FCU2['valve_pos_1'], id="fcu2_valve1_actuator", dbconfig=db_config)
actuator_3 = tb.SensorSystem(uuid=FCU2['valve_pos_2'], id="fcu2_valve2_actuator", dbconfig=db_config)
...
for i, sensor in enumerate(sensors):
    model.add_connection(sensor, controller, "measuredValue", f"sensorValue", input_port_index=i)

# Connect setpoints to controller
for i, setpoint in enumerate(setpoints):
    model.add_connection(setpoint, controller, "measuredValue", f"setpointValue", input_port_index=i)

# Connect controller outputs to actuators
for i, actuator in enumerate(actuator_sensors):
    model.add_connection(controller, actuator, f"inputSignal", "measuredValue", output_port_index=i)
...
for i, actuator in enumerate(actuator_sensors):
    ...
    actual = actuator.get_physical_readings(start_time=[start_time], end_time=[end_time], step_size=[step_size])
...
for i in range(len(actuator_sensors)):
    entry = [tb.plot.Entry(actual_values[i], label=f"Actuator {i}"), 
    tb.plot.Entry(initial_predictions[i], label=f"Initial Predictions {i}")]
    tb.plot.plot(simulator.date_time_steps, entry, title=f"Actuator {i} Initial Predictions vs Actual", ylabel_1axis="Position")
...
print(f"  - Beta (sensor selection): {n_beta} ({len(actuator_sensors)} actuators × {len(sensors)} sensors)")
...
fig, axes = plt.subplots(len(actuator_sensors), 1, figsize=(16, 4 * len(actuator_sensors)), sharex=True)
...
for i in range(len(actuator_sensors)):
    ...
    ax.plot(time_hours, actual_values[i], 'g-', alpha=0.7, linewidth=1.5, label='Actual')
    ax.plot(time_hours, initial_predictions[i], 'b--', alpha=0.5, linewidth=1, label='Initial')
...
plt.tight_layout()
plt.savefig('/mnt/user-data/outputs/fcu_identification_results.png', dpi=150, bbox_inches='tight')
