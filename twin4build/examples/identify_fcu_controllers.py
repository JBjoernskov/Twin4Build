...
for i, actuator in enumerate(actuator_sensors):
    ...
    print(f"  Actuator {i}: Initial MAE = {np.mean(np.abs(pred - actuator.input['measuredValue'].history(i_s=0, i_c=0).detach().numpy())):.4f}")
...
fig, axes = plt.subplots(len(actuator_sensors) + 1, 1, figsize=(16, 4 * (len(actuator_sensors) + 1)), sharex=True)
