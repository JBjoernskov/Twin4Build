...
all_sensors = [zone_temp_sensor, supply_air_flow_sensor, zone_temp_setpoint_sensor,
               damper_actuator, reheat_valve_actuator, supply_air_flow_cfm_sensor,
               ahu_supply_air_temp_sensor, ahu_supply_air_temp_sp_sensor,
               ahu_ccv_sensor, ahu_cooling_valve_sensor, ahu_heating_valve_sensor,
               ahu_outside_air_temp_sensor, ahu_return_air_temp_sensor]
...
parameters = model.components[f'identified_vav_controller_bldg13_{SELECTED_ROOM}'].get_estimator_parameters()
...
percent_air_flow_data = supply_air_flow_sensor.time_series_input.values[:,:,0].detach().numpy().T
supply_air_flow_cfm_data = supply_air_flow_cfm_sensor.time_series_input.values[:,:,0].detach().numpy().T
ahu_sat_plot_data = ahu_supply_air_temp_sensor.time_series_input.values[:,:,0].detach().numpy().T
...
for i in range(len(actuator_sensors)):
    entry = [
        ...
        tb.plot.Entry(percent_air_flow_data, label=f"Percent Air Flow (0-1)", linestyle="-."),
        ...
        tb.plot.Entry(temp_error_data, label=f"Temp Error (SP - actual)", axis=2, linestyle=":"),
        tb.plot.Entry(ahu_sat_plot_data, label=f"AHU Supply Air Temp", axis=2, linestyle="-."),
    ]
    tb.plot.plot(
...
cascade_a_setpoint = cascade_ctrl.ctrl_a.input["setpointValue"].history(i_c=0).detach().numpy().T
cascade_a_feedback = cascade_ctrl.ctrl_a.input["actualValue"].history(i_c=0).detach().numpy().T
cascade_a_output = cascade_ctrl.ctrl_a.output["inputSignal"].history(i_c=0).detach().numpy().T

# B-loop: setpoint (A-loop output = desired flow), feedback (weighted percent air flow), output (damper position)
cascade_b_setpoint = cascade_ctrl.ctrl_b.input["setpointValue"].history(i_c=0).detach().numpy().T
cascade_b_feedback = cascade_ctrl.ctrl_b.input["actualValue"].history(i_c=0).detach().numpy().T
cascade_b_output = cascade_ctrl.ctrl_b.output["inputSignal"].history(i_c=0).detach().numpy().T
...
entry_a = [
    ...
    tb.plot.Entry(cascade_a_error, label="A-loop Error (SP - feedback)", linestyle="-."),
    tb.plot.Entry(cascade_a_output, label="A-loop Output (intermediate flow SP)", axis=2),
]
tb.plot.plot(
    simulator.date_time_steps,
    entry_a,
...
entry_b = [
    ...
    tb.plot.Entry(actual_values[0], label="Actual Damper Position", axis=2, linestyle="--"),
]
tb.plot.plot(
    simulator.date_time_steps,
    entry_b,
...
entry_cascade = [
    ...
    tb.plot.Entry(cascade_b_feedback, label="% Air Flow (B-feedback)", axis=2, linestyle="-."),
    tb.plot.Entry(cascade_b_output, label="Damper Cmd (B-output)", axis=2, linestyle="--"),
]
tb.plot.plot(
    simulator.date_time_steps,
    entry_cascade,
...
ahu_sat_data = ahu_supply_air_temp_sensor.time_series_input.values[:,:,0].detach().numpy().T
ahu_sat_sp_data = ahu_supply_air_temp_sp_sensor.time_series_input.values[:,:,0].detach().numpy().T
...
fig.suptitle(f"Damper Position vs Potential Driving Signals - {SELECTED_ROOM}", fontsize=14)
...
fig2.suptitle(f"Temperature Error Distribution by Damper Mode - {SELECTED_ROOM}", fontsize=14)
...
for ax, (mask, label) in zip(axes2, modes):
    if np.sum(mask) > 0:
        ...
        ax.axvline(mean_err, color='green', linestyle='-', alpha=0.7, label=f"Mean = {mean_err:.2f}")
...
fig3.suptitle(f"Cross-Correlation: Damper Position vs Signals (lag in steps of {step_size}s) - {SELECTED_ROOM}", fontsize=13)
...
for ax, (signal, label) in zip(axes3.flat, xcorr_signals):
    ...
    ax.annotate(f"peak: lag={best_lag}, r={best_cc:.3f}",
                xy=(best_lag, best_cc), fontsize=9, ha='center',
...
ax4a.plot(time_axis, reheat_flat, label="Reheat Valve", linewidth=1, linestyle="--", alpha=0.7)
ax4a.set_ylabel("Position / Flow (0-1)")
ax4a.legend(loc="upper left", fontsize=9)
ax4a_r = ax4a.twinx()
ax4a_r.plot(time_axis, temp_error_flat, color='red', linewidth=1, alpha=0.6, label="Temp Error (SP-actual)")
...
ax4b_r = ax4b.twinx()
ax4b_r.plot(time_axis, zone_temp_flat, color='tab:orange', label="Zone Temp", linewidth=1)
ax4b_r.plot(time_axis, zone_sp_flat, color='tab:orange', linestyle='--', label="Zone Temp SP", linewidth=1)
ax4b_r.plot(time_axis, ahu_sat_flat, color='tab:green', label="AHU Supply Air Temp", linewidth=1)
ax4b_r.plot(time_axis, ahu_sat_sp_flat, color='tab:green', linestyle='--', label="AHU SAT Setpoint", linewidth=1, alpha=0.7)
...
ahu_cooling_valve_data = ahu_cooling_valve_sensor.time_series_input.values[:,:,0].detach().numpy().T
ahu_heating_valve_data = ahu_heating_valve_sensor.time_series_input.values[:,:,0].detach().numpy().T
ahu_oat_data = ahu_outside_air_temp_sensor.time_series_input.values[:,:,0].detach().numpy().T
ahu_rat_data = ahu_return_air_temp_sensor.time_series_input.values[:,:,0].detach().numpy().T
...
ax_a1.plot(time_axis, ahu_sat_flat, label="AHU Supply Air Temp", linewidth=1.5, color='tab:blue')
ax_a1.plot(time_axis, ahu_sat_sp_flat, label="AHU SAT Setpoint", linewidth=1.5, color='tab:blue', linestyle='--')
ax_a1.set_ylabel("Temperature (°C)")
ax_a1.legend(loc="upper left", fontsize=9)
ax_a1.set_title("AHU Supply Air Temp vs Setpoint")
ax_a1_r = ax_a1.twinx()
ax_a1_r.plot(time_axis, ahu_sat_error_flat, color='red', linewidth=1, alpha=0.7, label="SAT Error (SP - actual)")
...
ax_a2.plot(time_axis, ahu_ccv_flat, label="CCV (Cooling Coil Valve)", linewidth=1.2, color='tab:cyan')
ax_a2.plot(time_axis, ahu_cooling_valve_flat, label="Cooling Valve Output", linewidth=1.2, color='tab:blue', linestyle='--')
ax_a2.plot(time_axis, ahu_heating_valve_flat, label="Heating Valve Output", linewidth=1.2, color='tab:red')
...
ax_a2_r = ax_a2.twinx()
ax_a2_r.plot(time_axis, ahu_oat_flat, color='tab:green', linewidth=1.2, label="Outside Air Temp")
ax_a2_r.plot(time_axis, ahu_rat_flat, color='tab:orange', linewidth=1.2, label="Return Air Temp", linestyle='-.')
...
fig_c.suptitle(f"Cross-Correlation: Damper Position vs AHU Signals (lag in steps of {step_size}s) - {SELECTED_ROOM}", fontsize=13)
...
for ax, (signal, label) in zip(axes_c.flat, ahu_xcorr_signals):
    ...
    best_cc = cc[np.argmax(np.abs(cc))]
    ax.annotate(f"peak: lag={best_lag}, r={best_cc:.3f}",
                xy=(best_lag, best_cc), fontsize=8, ha='center',
                xytext=(0, 10), textcoords='offset points')
...
ax_d.fill_between(time_axis, 0, ahu_heating_valve_flat, alpha=0.3, color='tab:red', label="Heating Valve")
ax_d.fill_between(time_axis, 0, ahu_cooling_valve_flat, alpha=0.3, color='tab:blue', label="Cooling Valve Output")
ax_d.plot(time_axis, ahu_ccv_flat, linewidth=1.2, color='tab:cyan', label="CCV (Cooling Coil Valve)")
...
ax_d_r = ax_d.twinx()
ax_d_r.plot(time_axis, ahu_sat_flat, color='tab:green', linewidth=1.5, label="AHU Supply Air Temp")
ax_d_r.plot(time_axis, ahu_sat_sp_flat, color='tab:green', linewidth=1.5, linestyle='--', label="AHU SAT Setpoint")
ax_d_r.plot(time_axis, ahu_oat_flat, color='tab:orange', linewidth=1, linestyle='-.', label="Outside Air Temp")
...
ax_e.plot(time_axis, damper_flat, linewidth=1.8, label=f"{SELECTED_ROOM} Damper (selected)", color='black')
...
for _ri, _rname in enumerate(_comparison_rooms):
    ...
    try:
        ...
        if _rdf is not None and len(_rdf) > 0:
            ...
            _rvals = _rvals[:n_pts]
            ax_e.plot(np.arange(len(_rvals)), _rvals, linewidth=1, alpha=0.7,
                      color=_cmap_rooms(_ri), label=f"{_rname} Damper")
        else:
...
ax_e_r = ax_e.twinx()
ax_e_r.plot(time_axis, ahu_sat_flat, color='tab:green', linewidth=1.5, label="AHU Supply Air Temp")
ax_e_r.plot(time_axis, ahu_sat_sp_flat, color='tab:green', linewidth=1.5, linestyle='--', label="AHU SAT Setpoint")
...
print(f"    Heating valve active (> 5%):  {np.sum(_htg_active):4d} ({100*np.mean(_htg_active):.1f}%)")
print(f"    Cooling valve active (> 5%):  {np.sum(_clg_active):4d} ({100*np.mean(_clg_active):.1f}%)")
print(f"    CCV active (> 5%):            {np.sum(_ccv_active):4d} ({100*np.mean(_ccv_active):.1f}%)")
print(f"    Both heating & cooling off:   {np.sum(_both_off):4d} ({100*np.mean(_both_off):.1f}%)")

print(f"\n  AHU valve mean positions (when active):")
if np.sum(_htg_active) > 0:
    print(f"    Heating valve (when active): {np.mean(ahu_heating_valve_flat[_htg_active]):.3f}")
if np.sum(_clg_active) > 0:
    print(f"    Cooling valve (when active): {np.mean(ahu_cooling_valve_flat[_clg_active]):.3f}")
if np.sum(_ccv_active) > 0:
    print(f"    CCV (when active):           {np.mean(ahu_ccv_flat[_ccv_active]):.3f}")

print(f"\n  Temperature summary:")
print(f"    Outside Air Temp:  mean={np.mean(ahu_oat_flat):.1f}°C, min={np.min(ahu_oat_flat):.1f}°C, max={np.max(ahu_oat_flat):.1f}°C")
print(f"    Return Air Temp:   mean={np.mean(ahu_rat_flat):.1f}°C, min={np.min(ahu_rat_flat):.1f}°C, max={np.max(ahu_rat_flat):.1f}°C")
print(f"    AHU Supply Air:    mean={np.mean(ahu_sat_flat):.1f}°C, min={np.min(ahu_sat_flat):.1f}°C, max={np.max(ahu_sat_flat):.1f}°C")
print(f"    AHU SAT Setpoint:  mean={np.mean(ahu_sat_sp_flat):.1f}°C, min={np.min(ahu_sat_sp_flat):.1f}°C, max={np.max(ahu_sat_sp_flat):.1f}°C")
...
print(f"    Partial correlation: damper vs temp_error  r = {r_partial_damper_temperr:+.4f}  (AHU SAT removed)")
print(f"    Raw correlation:     damper vs pct_flow    r = {r_raw_damper_flow:+.4f}")
print(f"    Partial correlation: damper vs pct_flow    r = {r_partial_damper_flow:+.4f}  (AHU SAT removed)")
...
fig_t1.suptitle(f"Test 1: Partial Correlation (AHU SAT effect removed) - {SELECTED_ROOM}", fontsize=13)

# Scatter: raw damper vs temp error
axes_t1[0].scatter(temp_error_flat, damper_flat, c=time_color, cmap='viridis', s=10, alpha=0.7)
...
axes_t1[1].scatter(temp_error_resid, damper_resid, c=time_color, cmap='viridis', s=10, alpha=0.7)
...
axes_t1[2].scatter(pct_flow_resid, damper_resid, c=time_color, cmap='viridis', s=10, alpha=0.7)
...
print(f"    Damper vs Pct Airflow:  r = {r_damper_flow:+.4f},  R² = {r2_damper_flow:.4f}")
...
if r2_damper_flow > 0.90:
    ...
    print(f"        The inner flow loop is actively correcting, consistent with cascade.")
...
axes_t2[0].scatter(damper_flat, pct_flow_flat, c=time_color, cmap='viridis', s=10, alpha=0.7)
_x_line = np.linspace(np.min(damper_flat), np.max(damper_flat), 100)
axes_t2[0].plot(_x_line, np.polyval(_coeffs_df, _x_line), 'r-', linewidth=2, label=f"Linear fit (R²={r2_damper_flow:.3f})")
...
print(f"    SAT stability threshold: rolling_std <= {_std_threshold:.3f}°C (25th percentile)")
...
if n_stable >= 10:
    r_stable_damper_temperr = np.corrcoef(damper_flat[_stable_mask], temp_error_flat[_stable_mask])[0, 1]
    r_stable_damper_flow = np.corrcoef(damper_flat[_stable_mask], pct_flow_flat[_stable_mask])[0, 1]
    r_stable_damper_sat = np.corrcoef(damper_flat[_stable_mask], ahu_sat_flat[_stable_mask])[0, 1]

    print(f"\n    Correlations during AHU-SAT-stable windows:")
    print(f"      damper vs temp_error:  r = {r_stable_damper_temperr:+.4f}  (zone-level signal)")
    print(f"      damper vs pct_flow:    r = {r_stable_damper_flow:+.4f}  (inner loop)")
    print(f"      damper vs AHU SAT:     r = {r_stable_damper_sat:+.4f}  (should be weaker)")

    if abs(r_stable_damper_temperr) > abs(r_raw_damper_temperr) + 0.05:
        print(f"\n    --> RESULT: Temp error correlation STRONGER in stable windows")
        print(f"        ({r_stable_damper_temperr:+.3f} vs {r_raw_damper_temperr:+.3f}).")
    ...
    fig_t3.suptitle(f"Test 3: Conditional Analysis (AHU-SAT-stable windows) - {SELECTED_ROOM}", fontsize=13)

    # Highlight stable windows on a time series
    axes_t3[0].plot(time_axis, ahu_sat_flat, linewidth=1, alpha=0.5, label="AHU SAT")
    axes_t3[0].fill_between(time_axis, np.min(ahu_sat_flat), np.max(ahu_sat_flat),
                             where=_stable_mask, alpha=0.3, color='green', label="Stable windows")
    ...
    axes_t3[1].scatter(temp_error_flat[_stable_mask], damper_flat[_stable_mask],
                        c=time_color[_stable_mask], cmap='viridis', s=15, alpha=0.7)
    ...
    axes_t3[2].scatter(pct_flow_flat[_stable_mask], damper_flat[_stable_mask],
                        c=time_color[_stable_mask], cmap='viridis', s=15, alpha=0.7)
...
_n_bins = 8
_te_bins = np.linspace(np.percentile(temp_error_flat, 2), np.percentile(temp_error_flat, 98), _n_bins + 1)
_bin_centers = []
_bin_mean_flow = []
_bin_std_flow = []
_bin_mean_damper = []
...
for _bi in range(_n_bins):
    _mask_bin = (temp_error_flat >= _te_bins[_bi]) & (temp_error_flat < _te_bins[_bi + 1])
...
if len(_valid_flow) >= 3:
    ...
    print(f"    Spearman rank correlation (temp_error bins vs mean airflow): rho = {_rho:+.4f}, p = {_pval:.4f}")

    # Also check the flow range across bins
    _flow_range = np.max(_valid_flow) - np.min(_valid_flow)
    print(f"    Airflow range across bins: {_flow_range:.4f} (min={np.min(_valid_flow):.4f}, max={np.max(_valid_flow):.4f})")
...
axes_t4[0].bar(_bin_centers, _bin_mean_flow, width=(_te_bins[1]-_te_bins[0])*0.8,
               yerr=_bin_std_flow, color=_bar_colors, edgecolor='black', capsize=3, alpha=0.7)
axes_t4[0].set_xlabel("Temp Error: SP - actual (°C)")
axes_t4[0].set_ylabel("Mean Percent Airflow (0-1)")
axes_t4[0].set_title(f"Mean Airflow per Temp Error Bin (rho={_rho:+.3f})" if not np.isnan(_rho) else "Mean Airflow per Temp Error Bin")
# Add count labels
for _xi, _yi, _ci in zip(_bin_centers, _bin_mean_flow, _bin_counts):
    if not np.isnan(_yi):
        axes_t4[0].annotate(f"n={_ci}", xy=(_xi, _yi), fontsize=7, ha='center',
                            xytext=(0, 8), textcoords='offset points')

# Bar chart: mean damper per temp error bin
axes_t4[1].bar(_bin_centers, _bin_mean_damper, width=(_te_bins[1]-_te_bins[0])*0.8,
...
axes_t4[2].scatter(temp_error_flat, pct_flow_flat, c=ahu_sat_flat, cmap='coolwarm', s=8, alpha=0.5)
...
# Overall verdict
_cascade_score = 0
if abs(r_partial_damper_temperr) > 0.2: _cascade_score += 1
if r2_damper_flow > 0.85: _cascade_score += 1
if not np.isnan(r_stable_damper_temperr) and abs(r_stable_damper_temperr) > 0.2: _cascade_score += 1
if not np.isnan(_rho) and abs(_rho) > 0.5: _cascade_score += 1
...
if _cascade_score >= 3:
    ...
    print("  --> NO evidence for cascade control; damper may use direct PID or another scheme")
...
lambda_schedule = [
    ...
    (0.0,   {"maxiter": 100, "ftol": 1e-12, "disp": True}),   # Phase 1: pure fit, no penalty
...
sensor_names = ["zone_temp (sensor 0)", "percent_air_flow (sensor 1)", "AHU_supply_air_temp (sensor 2)"]
setpoint_names = ["zone_temp_setpoint (setpoint 0)"]

print("\nALPHA WEIGHTS (Candidate Selection):")
candidate_names = ["PID (reverse)", "PID (non-reverse)", "Cascade PID", "SAT-Compensated cascade"]
for a in range(len(actuator_sensors)):
