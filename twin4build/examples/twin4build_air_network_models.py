...
def solve_equilibrium_differentiable(
    model,
    fan_speed: torch.Tensor,
    ...
    for i in range(n_iterations):
        ...
        system_pressure = model.calculate_system_pressure(
            flow, supply_damper_pos, exhaust_damper_pos
        )
        ...
        if verbose and i % 10 == 0:
            print(
                f"Iteration {i}: max error = {max_error:.6f} Pa, flow = {flow.mean().item():.4f} m³/s"
            )
        ...
        if abs(max_error - prev_error) < 1e-6:
            ...
            if stall_count > 10:
                if verbose:
                    print(
                        f"Stalled at iteration {i}, error = {max_error:.6f} Pa (good enough)"
                    )
                break
        else:
            stall_count = 0
        ...
        flow_scale = model.fan_system.design_flow / (
            model.fan_system.design_pressure + 1e-6
        )
...
class FanSystem(core.System, nn.Module):
    ...
    def __init__(
        self,
        design_pressure: float = 600.0,
        design_flow: float = 5.0,
        ...
        **kwargs,
        ...
        self.a_param = tps.Parameter(
            torch.tensor(a_init / 1000, dtype=torch.float64)
        )  # kPa
        ...
        self.input = {"fanSpeed": tps.Scalar(), "flowRate": tps.Scalar()}
        self.output = {"pressureRise": tps.Scalar(), "fanPower": tps.Scalar()}
    ...
    def get_pressure_rise(
        self, flow: torch.Tensor, speed_ratio: torch.Tensor
    ) -> torch.Tensor:
        """Calculate fan pressure rise (differentiable)"""
        flow_norm = flow / (speed_ratio + 1e-6)
        pressure_design = (
            self.a_param.get() * 1000
            + self.b_param.get() * flow_norm
            + self.c_param.get() * flow_norm**2
        )
        pressure_rise = pressure_design * speed_ratio**2
        return torch.clamp(pressure_rise, min=0.0)

    def get_power(
        self, flow: torch.Tensor, pressure_rise: torch.Tensor
    ) -> torch.Tensor:
        ...
        eta_fan = 0.65 * (
            1.0 - 0.3 * (flow_ratio - 1.0) ** 2 - 0.2 * (pressure_ratio - 1.0) ** 2
        )
...
class DamperSystem(core.System, nn.Module):
    ...
    def __init__(
        self,
        fully_open_resistance: float = 10.0,
        fully_closed_resistance: float = 1e5,
        **kwargs,
    ):
        super().__init__(**kwargs)
        nn.Module.__init__(self)
        ...
        self.log_R_open_param = tps.Parameter(
            torch.tensor(np.log(fully_open_resistance), dtype=torch.float64)
        )
        self.k_param = tps.Parameter(
            torch.tensor(
                np.log(fully_closed_resistance / fully_open_resistance),
                dtype=torch.float64,
            )
        )

        # Define inputs and outputs
        self.input = {"damperPosition": tps.Scalar(), "flowRate": tps.Scalar()}
        self.output = {"pressureDrop": tps.Scalar(), "resistance": tps.Scalar()}
...
class BranchSystem(core.System, nn.Module):
    ...
    def __init__(
        self,
        resistance_coeff: float = 15.0,
        flow_exponent: float = 2.0,
        linear_resistance: float = 0.0,
        has_damper: bool = True,
        damper_R_open: float = 10.0,
        damper_R_closed: float = 1e5,
        **kwargs,
    ):
        super().__init__(**kwargs)
        nn.Module.__init__(self)
        ...
        # Learnable parameters
        self.log_R_duct_param = tps.Parameter(
            torch.tensor(np.log(resistance_coeff), dtype=torch.float64)
        )
        self.R_linear_param = tps.Parameter(
            torch.tensor(linear_resistance, dtype=torch.float64)
        )

        # Define inputs and outputs
        self.input = {"flowRate": tps.Scalar()}
        self.output = {"pressureDrop": tps.Scalar(), "ductPressureDrop": tps.Scalar()}

        # Add damper if specified
        if has_damper:
            self.damper = DamperSystem(
                damper_R_open, damper_R_closed, id=f"damper_{self.id}"
            )
    ...
    def get_pressure_drop(
        self, flow: torch.Tensor, damper_position: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        """Calculate branch pressure drop"""
        abs_flow = torch.abs(flow)
        R_duct = torch.exp(self.log_R_duct_param.get())
        dp_duct = (
            R_duct * abs_flow**self.flow_exponent
            + torch.abs(self.R_linear_param.get()) * abs_flow
        )
    ...
    def do_step(
        ...
        total_dp, duct_dp, damper_dp = self.get_pressure_drop(
            flow_rate, damper_position
        )
...
class AirNetworkSystem(core.System, nn.Module):
    ...
    def __init__(
        self,
        fan_config: Dict[str, Any] = None,
        supply_branch_configs: List[Dict[str, Any]] = None,
        exhaust_branch_configs: List[Dict[str, Any]] = None,
        **kwargs,
    ):
        super().__init__(**kwargs)
        nn.Module.__init__(self)
    ...
    def distribute_flow_parallel(
        self,
        total_flow: torch.Tensor,
        branches: nn.ModuleList,
        damper_positions: torch.Tensor,
        n_iterations: int = 15,
        ...
        if damper_positions.dim() == 1:
            ...
            damper_positions = damper_positions.unsqueeze(0).unsqueeze(
                0
            )  # scalar -> (1,1)

        batch_size = max(total_flow.shape[0], damper_positions.shape[0])
        n_branches = (
            damper_positions.shape[1] if damper_positions.dim() == 2 else len(branches)
        )
    ...
    def solve_network_equilibrium(
        ...
        system_pressure = self.calculate_system_pressure(
            flow_pos, supply_damper_pos, exhaust_damper_pos
        )
    ...
    def do_step(
        ...
        results = self.solve_network_equilibrium(
            fan_speed, supply_damper_pos, exhaust_damper_pos
        )
...
def create_air_network_model(
    fan_config: Dict[str, Any] = None,
    supply_configs: List[Dict[str, Any]] = None,
    exhaust_configs: List[Dict[str, Any]] = None,
    input_data: Dict[str, Any] = None,
    model_id: str = "air_network_model",
) -> tb.Model:
    """
    ...
    if input_data is not None:
        ...
        if "supply_dampers" in input_data:
            ...
            for i, col in enumerate(df.columns):
                damper_input = tb.TimeSeriesInputSystem(
                    df=pd.DataFrame({col: df[col]}), id=f"SupplyDamper{i}Input"
                )
                model.add_connection(
                    damper_input,
                    air_network,
                    "value",
                    "supplyDamperPositions",
                    input_port_index=i,
                )
        ...
        if "exhaust_dampers" in input_data:
            ...
            for i, col in enumerate(df.columns):
                damper_input = tb.TimeSeriesInputSystem(
                    df=pd.DataFrame({col: df[col]}), id=f"ExhaustDamper{i}Input"
                )
                model.add_connection(
                    damper_input,
                    air_network,
                    "value",
                    "exhaustDamperPositions",
                    input_port_index=i,
                )
    else:
        ...
        model.add_component(air_network)
        print(
            "Warning: No input data provided. You'll need to connect inputs manually or use for parameter estimation."
        )
...
def create_air_network_for_estimation(
    fan_config: Dict[str, Any] = None,
    supply_configs: List[Dict[str, Any]] = None,
    exhaust_configs: List[Dict[str, Any]] = None,
    measurement_data: Dict[str, pd.DataFrame] = None,
    model_id: str = "air_network_estimation",
) -> tb.Model:
    """
    ...
    if "fan_speed" in inputs_df.columns:
        fan_input = tb.TimeSeriesInputSystem(
            df=pd.DataFrame({"fanSpeed": inputs_df["fan_speed"]}), id="FanSpeedInput"
        )
        model.add_connection(fan_input, air_network, "value", "fanSpeed")
    ...
    if supply_cols:
        ...
        for i, col in enumerate(supply_cols):
            damper_input = tb.TimeSeriesInputSystem(
                df=pd.DataFrame({col: supply_df[col]}), id=f"SupplyDamper{i}Input"
            )
            model.add_connection(
                damper_input,
                air_network,
    ...
    exhaust_cols = [
        col for col in inputs_df.columns if col.startswith("exhaust_damper")
    ]
    if exhaust_cols:
        ...
        for i, col in enumerate(exhaust_cols):
            damper_input = tb.TimeSeriesInputSystem(
                df=pd.DataFrame({col: exhaust_df[col]}), id=f"ExhaustDamper{i}Input"
            )
            model.add_connection(
                damper_input,
                air_network,
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if "measured_flow" in measurements_df.columns:
            ...
            pd.DataFrame({"measuredValue": measurements_df["measured_flow"]}).to_csv(
                flow_file
            )
        ...
        if "measured_power" in measurements_df.columns:
            ...
            pd.DataFrame({"measuredValue": measurements_df["measured_power"]}).to_csv(
                power_file
            )
...
def generate_synthetic_data(n_samples: int = 300) -> Dict[str, Any]:
    ...
    measurements_df = pd.DataFrame(
        {"measured_flow": measured_flows, "measured_power": measured_powers},
        index=time_index,
    )
...
def run_parameter_estimation(measurement_data: Dict[str, Any]) -> tb.Model:
    ...
    print(
        f"  Fan: P_design={initial_fan_config['design_pressure']} Pa, Q_design={initial_fan_config['design_flow']} m³/s"
    )
    print(
        f"  Supply resistances: {[c['resistance_coeff'] for c in initial_supply_configs]}"
    )
    print(
        f"  Exhaust resistances: {[c['resistance_coeff'] for c in initial_exhaust_configs]}"
    )
    ...
    parameters = [
        # Fan parameters
        (
            air_network.fan_system,
            "a_param",
            initial_fan_config["max_pressure"] / 1000,
            0.1,
            2.0,
        ),
        ...
        (
            air_network.exhaust_branches[0],
            "log_R_duct_param",
            np.log(12),
            np.log(1.0),
            np.log(1000.0),
        ...
        (
            air_network.exhaust_branches[1],
            "log_R_duct_param",
            np.log(15),
            np.log(1.0),
            np.log(1000.0),
        ...
        (
            air_network.exhaust_branches[2],
            "log_R_duct_param",
            np.log(18),
            np.log(1.0),
            np.log(1000.0),
    ...
    print(
        f"  Fan curve: a={estimated_fan_a:.3f}, b={estimated_fan_b:.1f}, c={estimated_fan_c:.1f}"
    )
    print(f"  Supply resistances: {[f'{r:.1f}' for r in estimated_supply_resistances]}")
    print(
        f"  Exhaust resistances: {[f'{r:.1f}' for r in estimated_exhaust_resistances]}"
    )
    ...
    print(f"\nTrue parameters:")
    print(
        f"  Supply resistances: {[c['resistance_coeff'] for c in true_params['supply_configs']]}"
    )
    print(
        f"  Exhaust resistances: {[c['resistance_coeff'] for c in true_params['exhaust_configs']]}"
    )
...
def validate_estimation(model: tb.Model, measurement_data: Dict[str, Any]):
    ...
    try:
        ...
        axes[0, 0].plot(
            [measured_flow.min(), measured_flow.max()],
            [measured_flow.min(), measured_flow.max()],
            "r--",
            label="Perfect",
        )
        ...
        # Power prediction
        axes[0, 1].scatter(
            measured_power, estimated_power, alpha=0.6, s=20, color="orange"
        )
        axes[0, 1].plot(
            [measured_power.min(), measured_power.max()],
            [measured_power.min(), measured_power.max()],
            "r--",
            label="Perfect",
        )
        ...
        axes[1, 0].plot(
            time_subset, measured_flow[:100], "k.", label="Measured", markersize=3
        )
        axes[1, 0].plot(
            time_subset, estimated_flow[:100], "r-", label="Estimated", linewidth=1
        )
...
def example_air_network_simulation():
    ...
    print(
        f"✓ Air network has {len(model.components['AirNetwork'].supply_branches)} supply branches"
    )
    print(
        f"✓ Air network has {len(model.components['AirNetwork'].exhaust_branches)} exhaust branches"
    )
